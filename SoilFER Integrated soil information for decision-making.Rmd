...
This foundational module introduces R programming fundamentals specifically tailored for soil science applications. It consists of three sessions covering essential skills for soil data management and analysis.
...
Focuses on systematic soil data quality assessment and cleaning procedures essential before spatial modeling. Topics include coordinate validation, depth sequence checking, outlier detection and treatment, handling missing values, texture validation (sand-silt-clay constraints), duplicate resolution, and data harmonization techniques including depth standardization using mass-preserving splines.

**Session 1.2.1 - Working with Spatial Data in R**  
Covers fundamental concepts of spatial data handling using `{sf}` (for vector data) and `{terra}` (for raster data) packages. Participants learn coordinate reference systems, spatial transformations, geometric operations, raster processing, data visualization and methods for extracting environmental covariate values at soil sampling locations.

**Dataset**: KSSL (Kellogg Soil Survey Laboratory) georeferenced soil data from Kansas, USA - 10,352 soil horizon measurements from 2,584 profiles with laboratory analyses and spectroscopy data.

---

#### **Module 2: Soil Sampling Design** {-}

This module introduces the sampling methodology for soil surveys adopted in SoilFER.The module introduces SoilFER sampling procotol using the relationship between soils and the environmental diversity. The module presents the full process to create a proper sampling design according too such protocol.

**Session 2.1. Sampling Design fundamentals**  
- Soil forming factors and environmental variation: The SCORPAN model 
- Optimization of the sample size using the Kullback-Leibler Divergence.
- Use of Covariate Space Coverage (CSC) sampling for the identification of representative locations for soil samples
- Three-stage hierarchical SoilFER sampling design: Primary Sampling Units (PSUs - 2km × 2km), Secondary Sampling Units (SSUs - 100m × 100m), and Tertiary Sampling Units (TSUs - 20m × 20m)

**Session 2.2. Practical Implementation:**
The module includes a comprehensive **hands on** exercise for:
- Preparing environmental covariates
- Calculation of minimum sample size for describing soils using the SCORPAN background.
- Implementing Covariate Space Coverage (CSC) sampling for the identification of Primary Sampling Units
- Generating Secondary and Tertiary Sampling Units (SSUs and TSUs) using stratified random sampling.
- Creating site identification codes.
...
This module covers the Global Soil Information System (GloSIS) framework including the GloSIS database specifications. These framework enables the publication, global data interoperability and sharing of soil data. Participants learn to prepare soil data compliant with international standards, making it shareable through global soil information systems and compatible with FAO reporting requirements.

**Session 3.1. Harmonization of Soil Data for GloSIS**  
- Understanding ISO-28250 standards and the GloSIS database structure
- Features on Interest in the GloSIS database model
- Soil property units and measurement methods in GloSIS
- The GloSIS template for Soil Data Harmonization
- Automated harmonization tools for Soil Data.

**Session 3.2. GloSIS web services**  
- Introduction to GloSIS web services
- Docker tools for GloSIS deployment
- ETL for ingestion of soil data into PostgreSQL
- Metadata catalog service
- Web mapping services (OpenLayers) for data visualization

#### **Module 4: Digital Soil Mapping** {-}

This module provides comprehensive training in digital soil mapping (DSM) theory and practice, progressing from fundamental concepts through practical implementation of predictive soil mapping workflows.
The training focus on mapping SOC (Soil Organic Carbon), pH and texture (clay, silt, sand) across Kansas using the standardized KSSL dataset produced in Module 1.

**Session 4.1. Data preparation and theoretical background for DSM**  
- **Feature selection**: Identifying relevant covariates to explain spatial differences in the target parameters 
- **Machine learning algorithms**: 
  - Random Forest for non-linear relationships and feature importance
  - Gradient Boosting Machines (GBM) for high-accuracy predictions
  - Support Vector Machines for classification tasks
  - Ensemble methods combining multiple models
- **Model training and tuning**: Cross-validation, hyperparameter optimization, preventing overfitting
- **Spatial prediction**: Generating wall-to-wall maps of soil properties across landscapes
- **Uncertainty quantification**: Prediction intervals, confidence maps, and model reliability assessment
- **Map validation**: Independent validation, spatial cross-validation, accuracy metrics (RMSE, R², CCC)

**Session 4.2. Practical Workflows:**
- Extracting relevant covariate values at soil sample locations
- Data splitting strategies for training and validation
- Building prediction models for:
  - Continuous Variables (pH)
  - Nominal Classes (unordered soil classes combining clay content and pH categories)
  - Ordinal Classes (ordered classes of soil pH - `very_acid < acid < neutral < alkaline < very_alkaline`)
  - Raster predictive maps of soil parameters and its associated accuracy


#### **Module 5: Soil Spectroscopy** {-}

This module introduces soil infrared spectroscopy as a rapid, cost-effective method for predicting soil properties, and its integration into DSM workflows.
The module uses the KSSL dataset's wet chemistry analyses (14 soil properties) linked to mid-infrared spectral scans, providing a real-world example of spectroscopy-DSM integration.

**Session 5.1. Spectroscopy fundamentals**
- **Spectral fundamentals and data preprocessing**: Baseline correction, smoothing, derivative transformations, scatter correction
- **Quality control**: Spectral outlier detection, replicate measurement analysis
- **Chemometric modeling**: Partial Least Squares Regression (PLSR), support vector regression, random forest for spectral calibration
- **Spectral library development**: Building and maintaining reference spectral databases
- **Gap-filling strategies**: Using spectroscopy to predict properties with low laboratory analysis completeness


**Session 5.2. Practical Exercises:**
- Processing raw spectral scans from the KSSL dataset (4 replicates per sample)
- Building calibration models for SOC, texture, pH, and other properties
- Integration of spectral predictions into DSM workflows


#### **Module 6: Soil Data Sharing** {-}

This module covers the technical and organizational aspects of making soil data discoverable, accessible, and interoperable through web services and international data sharing platforms.

**Session 6.1: Data standards and formats**
- **Metadata creation**: ISO 19115/19139 standards for spatial metadata
- **Web service deployment**: Setting up OGC-compliant data services
- **Data licensing and attribution**: Creative Commons, open data principles, proper citation practices
- **Integration with global platforms**: Contributing to FAO GloSIS, ISRIC WoSIS (World Soil Information Service), national soil databases
- **API development**: RESTful APIs for programmatic data access

**Session 6.2: Technical Implementation:**
- Configuring web map servers for soil data visualization
- Publishing soil datasets through GeoServer or similar platforms
