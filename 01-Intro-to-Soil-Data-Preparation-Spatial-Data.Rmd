...
#####  Format Date to Standard: ISO 8601 (specifically mapped as YYYY-MM-DD)

Sampling dates are required for standardizing soil data for the GloSIS database structure. In this dataset, the sampling date is recorded only as a year in the `fiscal_year` column. To ensure compatibility with the Soil Information System (SIS), this information is converted to an ISO 8601-compliant date (`YYYY-MM-DD`) by assigning January 1st as the reference date for each fiscal year.

```{r add-date, eval=FALSE}
raw_data <- raw_data %>%
...
**Adjust the sampling `date` for the observations** 
   ...
   - If a proper `date` field is not available, a predefined criterion can be used to assign a reference date. For example, January 1st of the corresponding year can be used. Keep in mind that this is an approximate reference date and does not represent the actual sampling date.
   
   - Whenever the actual sampling `date` is known, it should be used instead of an approximate reference date.
   
   - If the `date` field already exists in the dataset, this step should be skipped.

:::

##### Adding unique record and profile identifiers

During data cleaning, records may be removed, modified, or reorganized. It is therefore good practice to assign a unique sequential `rowID` to each record in the original imported dataset. This identifier should be created before any filtering or transformation that could change the number or order of records.

The `rowID` allows each processed record to be traced back to its corresponding record in the original dataset. This improves transparency and reproducibility and makes it easier to identify which records were retained, modified, or excluded during the cleaning process.

A second identifier, `ProfID`, is created to identify soil profiles. In this dataset, all horizons sharing the same longitude, latitude, and sampling date are initially assumed to belong to the same soil profile description. The identifier is created once at this stage and is preserved throughout the subsequent site and laboratory data-cleaning steps.
...
After standardizing the column names, the `site` object preserves the `rowID` and `ProfID` created in `raw_data`. Each row therefore remains linked to the original record, while horizons belonging to the same initial profile share a common `ProfID`.

```{r reorder-site-columns, eval=FALSE}
# Reorder columns for clarity
site <- site %>%
  select(rowID, ProfID, HorID, date, lon, lat, top, bottom)
```

##### Retaining repeated horizon records until laboratory validation 

Multiple records may correspond to the same location and depth. In this dataset, repeated records may arise from spectroscopic measurements, repeated laboratory analyses, or multiple profile descriptions. At this stage, these records are **retained** rather than removed.
...
**Adjust `ProfID` for locations with multiple profile descriptions** 
  ...
  - In this workflow, `ProfID` is initially based on spatial position and sampling `date`, which separates observations collected at the same location on different dates. However, multiple independent profile descriptions may still occur at the same location and date.
  
  - These cases are identified later from their depth sequences and assigned distinct `ProfID` values in the section **Detecting and resolving duplicate soil profiles**.
...
At this stage, `site` contains the records that passed the coordinate and basic depth-structure checks. The next step does not reconstruct the profiles. Instead, it adds the corresponding analytical measurements to these already validated site records.

#### Preparing lab data: standardization and validation
...
At this point, the `site` object already contains the cleaned and validated profile structure. Therefore, the site preparation steps are **not repeated**. Instead, laboratory variables are extracted from `raw_data` using the original `rowID` and attached to the records that remain in `site`.

This preserves the profile identifiers, coordinates, dates, and depth intervals already validated in the previous sections. The resulting `site_lab` object is therefore a direct continuation of `site`: it contains the cleaned site structure plus the laboratory measurements that will now be validated.

```{r lab-extract, eval=FALSE}
# Extract laboratory measurements from the original data.
# rowID provides the link back to the already cleaned site object.
lab_data <- raw_data %>%
  select(
    rowID,
...
# The basic site checks (missing/negative depths, zero thickness, invalid
# depth logic, and initial surface coverage) have already been applied to `site`
# and inherited by `site_lab`.
# 1. For each profile, check whether rows form one continuous depth sequence
# 2. If YES → Single profile (done)
# 3. If NO → Find the consecutive horizons that are continuous
# 4. If we find separate blocks → Split the series into subprofiles
...
After this procedure, `site_lab` is the final clean horizon-level dataset derived sequentially from `site`. It contains validated site, analytical, and metadata information. The dataset contains:
