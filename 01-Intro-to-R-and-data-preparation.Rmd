...
Soil properties are not randomly distributed in space but change in close relationship with different features in the landscape.

Following Jenny´s equation of soil forming factors, soil is a function of environmental factors interacting over time [@prieto2023]:
...
These relationships have been tratitionally used by soil scientists to ascertain the distribution of different soil tyes in traditional soil mapping activities. 

These relationships can be also used in modern framewosk for soil mapping to model and predict variations in soil properties across the space.
...
To perform this conversion, users must specify the columns containing the coordinates:

```{r dataframe-to-sf, echo=TRUE, eval=FALSE}
library(sf)

# Create a dataframe with spatial coordinates (WGS84) within Kansas 
k.data <- data.frame(
  site_id = c("KS_01", "KS_02", "KS_03", "KS_04", "KS_05"),
  X = c(-96.70, -97.61, -95.24, -100.00, -94.90),  # longitude
  Y = c(39.03, 38.84, 37.69, 37.20, 39.10),        # latitude
  soc_gkg = c(18.2, 14.7, 21.5, 10.9, 16.3)         # example soil variable
)
...
In the example above, the Coordinate Reference System (CRS) as been defined as an argument within the  `st_as_sf()` function
...
- If the CRS is unknown or incorrectly specified, spatial operations such as buffering, distance, or overlay may fail or produce errors or misleading results.
...
This layer may represent individual soil pits or auger locations, with coordinates and measured properties stored in the attribute table.
...
The `{terra}` package provides the `rast()` function to read both single-layer and multi-layer raster files (e.g., GeoTIFF).

Single-layer raster: one variable (e.g., a cropland mask, DEM, SOC map).

Multi-layer raster: a stack/collection of variables in one file (e.g., multiple environmental covariates).
...
# Read single-layer raster tif file (binary cropland mask)
crops <- rast("Cropland_Mask_KANSAS.tif")

# Basic visualization
plot(crops)

# Read multi-layer raster tif file (environmental covariates stack)
covs <- rast("Env_Cov_250m_KANSAS.tif")

# Visualize the first four layers
plot(covs[1:4])
...
Before using rasters in analysis or modeling, it is important to inspect their spatial basic properties (extent, resolution, CRS), number of layers and value ranges. This helps identify issues such as mismatched CRS, unexpected resolution, incorrect extents, data reanges and missing data patterns.
...
# Quick summaries (min/max etc.)
global(crops, fun = "range", na.rm = TRUE)
global(covs[[1]], fun = "range", na.rm = TRUE)

# Simple histogram of values (single layer)
hist(covs[[1]], main = "Histogram of covariate layer 1", xlab = names(covs)[1])
```

:::warning-box
For most raster operations, rasters should share the same CRS, resolution, and extent. 
If they do not, they have to be adjusted typically with `resample`, `project`, or `crop` before combining.
:::
...
Adjacent raster tiles can be merged into a single, continuous layer using a mosaic operation. In the example below, two binary cropland mask tiles (1 = cropland presence; NA = NoData/no cropland) are combined into one raster. The resulting mosaic is then written as a compact GeoTIFF using an 8-bit unsigned integer datatype to reduce file size while maintaining the mask structure.
This operation can be used to merge raster tiles produced during the raster exportation in Google Earth Engine.
  
```{r crop-mosaic, echo=TRUE, eval=FALSE}
library(terra)

# Read adjacent cropland binary (1= presence, NA=absence) mask tiles
r1 <- rast("Cropland_Mask_KANSAS_1.tif")
r2 <- rast("Cropland_Mask_KANSAS_2.tif")

# Mosaic tiles (keep cropland presence where either tile has value 1)
m <- mosaic(r1, r2, fun = "max")

# Cast to integer (suitable for a Byte mask)
m <- as.int(m)

# Write a compact GeoTIFF (Byte + compression), preserving NA as NoData
writeRaster(
  m,
...
# Preparation of Environmental Covariates with Google Earth Engine

Sampling Design and Digital Soil Mapping relies heavily on environmental covariates that represent the soil-forming factors of climate, organisms, relief, parent material, and time (the SCORPAN framework). Assembling these covariates from global datasets such as CHELSA climate surfaces, SRTM terrain derivatives, MODIS vegetation indices, and Sentinel-2 spectral bands traditionally requires managing dozens of large files across different formats and projections. GEE provides direct access to all these datasets through a unified interface, allowing analysts to extract, compute, and export ready-to-use covariate stacks for any region of interest with a single script.

In the context of this training, we use GEE to extract and export a comprehensive set of environmental covariates for our study area (Kansas). The exported rasters are then used as covariates to:
...
## The SoilFER Environmental Covariates Script

The script provided for this training (`1_gee_soilfer_env_covariates.txt`) provides the instructions to download covariate data from the GEE platform. It assembles five categories of environmental covariates within a single, configurable workflow.
...
All outputs are exported as cloud-optimized GeoTIFF files to Google Drive at their native or user-specified resolutions, ready for import into R for the modeling workflow.
...
The `aoi` variable defines the `area of interest`. For the Kansas training exercise, this is already configured to use the US Census TIGER state boundary layer:
