...
Geographic Coordinate Systems are good for storing global data, but not suitable for distance, area, or buffering calculations because degrees are angular units, and the physical distance of one degree changes with latitude (Fig. \@ref(fig:degreeLat)) . This leads to distortions in distances areas calculations.
...
At its core, GEE combines three key elements: a multi-petabyte data catalog hosted on Google's servers, a high-performance parallel computing infrastructure, and a programming interface (API) accessible through both a JavaScript-based Code Editor and a Python client library. Users write analysis scripts that run directly on Google's servers, where the data already resides, meaning computationally intensive operations that would take days on a local machine can complete in minutes.

:::key-concept
**Why GEE for Digital Soil Mapping?**

Digital Soil Mapping relies heavily on environmental covariates that represent the soil-forming factors of climate, organisms, relief, parent material, and time (the SCORPAN framework). Assembling these covariates from global datasets such as CHELSA climate surfaces, SRTM terrain derivatives, MODIS vegetation indices, and Sentinel-2 spectral bands traditionally requires managing dozens of large files across different formats and projections. GEE provides direct access to all these datasets through a unified interface, allowing analysts to extract, compute, and export ready-to-use covariate stacks for any region of interest with a single script.
:::

In the context of this training, we use GEE to extract and export a comprehensive set of environmental covariates for our study area (Kansas), including CHELSA bioclimatic variables, TerraClimate time series, SRTM-derived terrain attributes, MODIS FPAR seasonality layers, Sentinel-2 spectral indices, ESA WorldCover land cover, and geomorphological features from Geomorpho90m. The exported rasters are then used as covariates to:
...
GEE requires a Google account. If you do not already have one, create a free account at [https://accounts.google.com](https://accounts.google.com). Use an institutional email address (university or research organization) if possible, as this can facilitate the approval process.
...
Once all exports are complete, download the GeoTIFF files from Google Drive to your local working directory. In subsequent modules, these rasters will be loaded into R using the `terra` package and stacked into the covariate array used for model training and spatial prediction.
