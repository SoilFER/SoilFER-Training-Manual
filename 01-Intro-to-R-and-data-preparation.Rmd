...
# raw_data <- read_excel(paste0(training_folder,"01_data/module1/KSSL_data.xlsx"), sheet = 1) 
# property_thresholds <- read_csv(paste0(training_folder,"01_data/module1/property_thresholds.csv"))
# kdata <- read.csv (paste0(output_dir,"KSSL_DSM_0-30.csv"))
# soil_sf <- st_read(paste0(output_dir,"soil_profiles.shp"))
# admin <- st_read(paste0(training_folder,"01_data/module1/Tiger_2020_Counties.shp"))
...
Thresholds are stored in a configuration file (`property_thresholds.csv`) to ensure transparency and reproducibility.
...
admin <- st_read("01_data/module1/Tiger_2020_Counties.shp")
...
**Soil Available Water Content** is represented by the Available Water Content layer estimated at 200 cm depth at....
...
Data in this folder includes:
 ...
 - [**sol_available.water.capacity_usda.mm_m_250m_0..200cm_1950..2017_v0.1.tif**][@gupta_zenodo_2019](https://drive.google.com/file/d/1xq6y8jNUjDbipXHQE1w67f_vxQeXACkH/view?usp=drive_link).  250-meter resolution geospatial raster dataset representing the mean available water capacity (AWC - in mm) of soil from 0 to 200 cm depth.
...
writeRaster(m,"Cropland_Mask_KANSAS.tif", overwrite = TRUE,
...
m <- as.int(m)
# Write output with compression to reduce file size
writeRaster(m,"ESA_WorldCover_2021_KANSAS.tif", overwrite = TRUE,
  wopt = list(datatype = "INT1U",  # Byte (0–255); NA retained as NoData
    gdal = c("COMPRESS=DEFLATE", "PREDICTOR=2", "ZLEVEL=9", "TILED=YES")
  )
)
...
writeRaster(crops, "Cropland_Mask_KANSAS_copy.tif", overwrite = TRUE) # overwrites tif
```
...
### Preparation of Soil–Climate Covariates (Newhall Simulation Model)
 ...
 * Available Water Capacity (AWC): to represent how much water the soil can store (typically for a reference depth, e.g., 0–200 cm), which controls how quickly soils dry after rainfall.
...
raster.path <- "rasters/" # Path to your raster covariates 
shp.path    <- "shapes/" # Path to your shapefiles 
...
tmp <- rast(file.path(raster.path, "GEE_Exports/TerraClimate_AvgTemp_1981_2023_KANSAS.tif"))
...
# Write to disk
writeRaster(Temp_Stack, file.path(raster.path, "Temp_Stack_monthlyMean.tif"), overwrite = TRUE)


# Monthly precipitation time series (multi-year, monthly layers)
prec_mm <- rast(file.path(raster.path, "GEE_Exports/TerraClimate_Precip_1981_2023_KANSAS.tif"))
# Number of years in the stack (assuming complete years
...
# Write to disk
writeRaster(Prec_Stack, file.path(raster.path, "Prec_Stack_monthlyMean.tif"), overwrite = TRUE)
```
...
Prec_Stack <- rast(file.path(raster.path, "Prec_Stack_monthlyMean.tif"))
Temp_Stack <- rast(file.path(raster.path, "Temp_Stack_monthlyMean.tif"))
...
writeRaster(combined_stack, file.path(raster.path, "climate_vars.tif"), overwrite = TRUE)
````
...
# ROI boundary
# Read ROI from chapefile
roi <- st_read(file.path(shp.path, "roi_kansas_us_epsg_4326.shp"), quiet = TRUE)
# Align CRS to the common EPSG
if(st_crs(roi)$wkt != st_crs(epsg)$wkt) roi <- st_transform(roi, epsg)

# DEM
# Define path to raster
elev_file <- list.files(raster.path, pattern = "DEM_KANSAS.tif$", recursive = TRUE, full.names = TRUE)
# Read raster
elev <- rast(elev_file)
# Align CRS to the common EPSG
...
# AWC (0–200 cm)
# Define path to raster
awc <- rast(file.path(raster.path, "sol_available.water.capacity_usda.mm_m_250m_0..200cm_1950..2017_v0.1.tif"))
# Align CRS to the common EPSG
if(crs(awc) != epsg) awc <- project(awc, epsg, method = "near")
# Crop raster to the limits of the Region-on-Interest
awc <- crop(awc, vect(roi))
...
# Read climate stack
newhall <- rast(file.path(raster.path, "climate_vars.tif"))
# Align CRS to the common EPSG
...
writeRaster(newhall, file.path(raster.path, "climate_newhall_vars.tif"), overwrite = TRUE)
```
...
newhall <- rast(file.path(raster.path, "climate_newhall_vars.tif"))
...
writeRaster(results, file.path(raster.path, "newhall.tif"), overwrite = TRUE)

# Optional compact integer exports (×10 preserves 1 decimal place)
results_climate_int <- app(newhall,  function(x) as.integer(x * 10))
writeRaster(results_climate_int, file.path(raster.path, "climate_intx10.tif"), overwrite = TRUE)

results_newhall_int <- app(results, function(x) as.integer(x * 10))
writeRaster(results_newhall_int, file.path(raster.path, "newhall_intx10.tif"), overwrite = TRUE)
```

The raster stack `newhall_results` contains the NSM outputs that can be used as covariates in both the Sampling Design and Digital Soil Mapping workflows.
