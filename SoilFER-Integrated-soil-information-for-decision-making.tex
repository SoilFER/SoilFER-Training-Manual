...
\textbf{Session 4.1 - Spectroscopy Fundamentals and Calibration}\\
...
\textbf{Session 4.2 - Practical Spectroscopy Workflows}\\
...
\subsubsection*{\texorpdfstring{\textbf{Module 5: Soil Data Sharing}}{Module 5: Soil Data Sharing}}\label{module-5-soil-data-sharing}
\addcontentsline{toc}{subsubsection}{\textbf{Module 5: Soil Data Sharing}}
...
\textbf{Session 5.1 - Standards, Metadata, and Interoperability}\\
...
\textbf{Session 5.2 - Web Services Deployment}\\
...
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/shp-to-sf-hidden-1.pdf}}
...
\begin{verbatim}
##     ProfID               top        bottom        Clay             Silt            Sand            SOC       
...
## 6        A  61987 +38.4819222 -101.3474341 c1ad0367-fe83-4ee2-91fb-256e33e36857 POINT (-101.5333 38.50486)
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{covs }\OtherTok{\textless{}{-}} \FunctionTok{rast}\NormalTok{(}\StringTok{"01\_data/module1/training\_data/Environmental\_Covariates\_250m\_KANSAS.tif"}\NormalTok{)}
...
The \texttt{eval()} function, loaded from \texttt{eval.RData}, implements the accuracy framework proposed by Wadoux \emph{et al.} (\citeproc{ref-Wadoux2021integrated}{2021}). It computes six complementary metrics from the vectors of observed (z) and predicted (ẑ) values:
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/tile1-mean-1.pdf}}
\caption{\label{fig:tile1-mean}SOC mean prediction for tile 1 (g/kg).}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/tile1-sd-1.pdf}}
...
There is broad consensus that DRS complements rather than replaces soil laboratory analyses Chen \emph{et al.} (\citeproc{ref-chen2023integrating}{2023}), increasing analytical throughput while retaining the validity of reference procedures. For DSM, this means that spectroscopy can expand soil property datasets and improve mapping workflows while reducing cost and logistical constraints. This is true when the empirical models that translate spectra into soil property predictions have been properly calibrated and validated. It is important to note, however, that developing these models can still be laborious, time-consuming, and costly, a limitation that is rarely acknowledged in the soil spectroscopy literature. In this respect, pre-existing soil spectral libraries containing reference samples offer an opportunity to reduce calibration effort when developing new models for a specific area or application.

\begin{quote}
\emph{DRS can be a time- and cost-effective tool for soil analysis, as numerous studies have reported. However, this is mainly true when suitable predictive models have already been calibrated and validated. When models need to be developed from scratch, the process of generating reference data, calibrating robust models, and validating their performance can itself be costly and time-consuming. This limitation is often insufficiently acknowledged in the soil spectroscopy literature (\citeproc{ref-RamirezLopez2026Liblex}{Ramirez-Lopez \emph{et al.}, 2026}).}
\end{quote}

\subsection{Rationale}\label{rationale}

The use of DRS-inferred soil properties offers a cost- and time-efficient pathway to substantially increase the number of observations available for DSM model calibration (\citeproc{ref-ramirez2019robust}{Ramirez-Lopez \emph{et al.}, 2019}). This directly supports the collection of spatially dense datasets at field to regional scales. By augmenting conventional laboratory datasets with larger volumes of DRS-inferred observations, DSM workflows can benefit from more input data, which may improve map accuracy at lower overall cost.

\begin{quote}
{Cost perspective:} DRS reduces costs mainly by lowering the marginal cost of additional soil observations. Laboratory analyses are still needed for calibration and validation, but once reliable models are available, each new spectrum can provide predictions for one or several soil properties. In repeated large-scale soil monitoring, the cost benefit depends on whether prediction errors are acceptable for the intended decision. For example, (\citeproc{ref-breure2026evaluating}{\textbf{breure2026evaluating?}}) assessed VNIRS for SOC prediction in a repeated European soil survey and showed that, under favourable assumptions, replacing (most of the) dry-combustion measurements with VNIRS in a second survey campaign could reduce analytical costs. This makes DRS especially useful in DSM workflows, where increasing sampling density and monitoring frequency can be more important than analysing every sample only by conventional methods, as also shown by (\citeproc{ref-malone2022digital}{\textbf{malone2022digital?}}).
\end{quote}
...
In DSM workflows that incorporate DRS, spectroscopically derived soil property estimates should be treated as model-based predictions with associated uncertainty, rather than as error-free measurements. Unlike conventional laboratory analyses, spectroscopy does not directly measure soil properties; it generates predictions from statistical models calibrated to capture relationships between spectral responses and reference measurements. Consequently, each DRS estimate is associated with uncertainty that is usually larger than that of conventional laboratory measurements. When DRS-inferred data are incorporated into DSM workflows, this additional source of uncertainty should therefore be explicitly acknowledged (\citeproc{ref-takoutsing2022accounting}{Takoutsing \emph{et al.}, 2022}). Treating DRS predictions as error-free observations may lead to biased spatial models, propagation of systematic spectroscopic errors, and overconfident DSM predictions (\citeproc{ref-van2022measurement}{Westhuizen \emph{et al.}, 2022}).

Different methodological strategies have been proposed to propagate or account for uncertainty in soil property observations used in DSM, depending on the modeling framework adopted. In geostatistical mapping, observation-level uncertainty in soil property data has been incorporated into spatial covariance structures (\citeproc{ref-ramirez2019robust}{Ramirez-Lopez \emph{et al.}, 2019}; \citeproc{ref-takoutsing2022accounting}{Takoutsing \emph{et al.}, 2022}). In machine-learning-based DSM, the literature remains more limited, although several studies have explored weighting schemes to reflect differences in data quality. For example, weights derived from DRS prediction uncertainty have been used in convolutional neural networks and random forest models by modifying the loss function during model calibration (\citeproc{ref-hengl2018random}{Hengl \emph{et al.}, 2018}; \citeproc{ref-wadoux2019multi}{Wadoux, Padarian and Minasny, 2019}). More recently, error-filtered machine learning frameworks have been proposed in which observation-level uncertainty is explicitly integrated into likelihood-based estimation, with improved predictive performance when observation errors are substantial (\citeproc{ref-van2022measurement}{Westhuizen \emph{et al.}, 2022}).
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-uncertainty-1} 

}

\caption{Conceptual illustration of measurement and prediction uncertainty. Narrow distributions represent wet-chemistry observations with lower analytical uncertainty, whereas wider distributions represent DRS predictions with larger associated standard deviations. Horizontal arrows indicate one standard deviation around the estimated value.}\label{fig:fig-uncertainty}
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{images/module4/fig_dsm_workflow} 

}

\caption{Conceptual workflow for integrating spectroscopically predicted soil data into digital soil mapping.}\label{fig:fig-dsm-workflow}
\end{figure}
...
\begin{itemize}
\item
  \texttt{MIR\_KANSAS\_data.xlsx}: an excel spreadsheet containing mid-infrared (MIR) spectra and associated soil reference measurements for soil samples collected across Kansas, USA. This file is loaded as the primary analytical dataset for spectral modelling and soil property prediction.
\item
  \texttt{Environmental\_Covariates\_250m\_KANSAS.tif}: a multi-band raster file containing environmental covariates at 250 m spatial resolution derived from remote sensing and climate products. These covariates follow the SCORPAN framework and are used as predictors for digital soil mapping across the Kansas study area.
\end{itemize}

Both files must be placed in the \texttt{01\_data/module1/training\_data/} folder of the project directory before running any code in this module, as all scripts reference that location via relative paths. Refer to Section \ref{required-downloads} for step-by-step download and setup instructions.

\subsection{Main R package sused here}\label{main-r-package-sused-here}

The main R packages used in this section are:

\begin{itemize}
\item
  \texttt{prospectr}: used for handling and preprocessing the DRS spectra to enhance their quality. This package is also used for representative sample selection.
\item
  \texttt{ranger}: used to fit random forest and quantile regression forest models. In this section, it is used both to predict SOC from DRS spectra and to estimate prediction uncertainty.
\item
  \texttt{aqp}: used to represent soil profiles and horizons in a structured way, allowing SOC values and their uncertainties to be harmonized to a standard depth interval.
\item
  \texttt{terra}: used to handle spatial raster data, including environmental covariates, extraction of covariate values at soil sampling locations, and prediction of SOC maps.
\end{itemize}

\subsection{Load and plot the DRS data}\label{load-and-plot-the-drs-data}

First, we import the DRS dataset and prepare it for analysis. This dataset contains absorbance measurements collected in the mid-infrared (MIR) region of the electromagnetic spectrum, over the wavenumber range starting from apprximately 4000 cm\(^{-1}\) up to apprximately 400 cm\(^{-1}\) at an approximated resolution of 1.93 cm\(^{-1}\). This resulted in 1764 spectral variables. Note that this region corresponds to wavelengths of approximately 2500--25000 nm. These measurements were collected under laboratory conditions using air-dried soil samples. Additional details on the DRS measurements are provided in Dangal \emph{et al.} (\citeproc{ref-dangal2019accurate}{2019}).

\begin{quote}
{NOTE:} \emph{MIR spectra show strong, well-defined absorption features associated with molecular vibrations in soil minerals and organic compounds. This makes MIR spectroscopy sensitive to properties such as soil organic carbon, clay minerals, carbonates, and some nutrients.
Although this example uses MIR spectra, the workflow can also be applied to vis--NIR or SWIR data, provided suitable calibration models and uncertainty estimates are available.}
\end{quote}
...
\NormalTok{original\_dat }\OtherTok{\textless{}{-}} \FunctionTok{read\_excel}\NormalTok{(}
  \StringTok{"../SoilFER{-}Training{-}Resources/01\_data/module1/training\_data/MIR\_KANSAS\_data.xlsx"}\NormalTok{,}
  \AttributeTok{.name\_repair =} \StringTok{"minimal"}\NormalTok{,}
  \AttributeTok{na =} \FunctionTok{c}\NormalTok{(}\StringTok{""}\NormalTok{, }\StringTok{"NA"}\NormalTok{, }\StringTok{"N/A"}\NormalTok{, }\StringTok{"NaN"}\NormalTok{),}
  \AttributeTok{progress =} \ConstantTok{FALSE}
\NormalTok{)}

\NormalTok{original\_dat }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(original\_dat)}

\CommentTok{\# dimensions of the dataset (number of samples and variables)}
\FunctionTok{dim}\NormalTok{(original\_dat)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# number of duplicate sample IDs (i.e. repeated measurements for }
\CommentTok{\# the same soil sample)a}
\FunctionTok{sum}\NormalTok{(}\FunctionTok{duplicated}\NormalTok{(original\_dat}\SpecialCharTok{$}\NormalTok{smp\_id))}
\end{Highlighting}
\end{Shaded}
...
The spectral variables can be identified from the column names. These can be inspected using \texttt{colnames(dat)}. In this dataset, spectral columns follow a consistent naming pattern: they start with the prefix \texttt{X}, followed by numeric values representing the original wavenumbers. For example, a column named \texttt{X3999.7} corresponds to a spectral variable measured at approximately 3999.7 cm\(^{-1}\).

We use this pattern to separate the spectral variables from the remaining soil data. The \texttt{X} prefix is then removed so that the column names can be converted into numeric wavenumber values. These values are stored in \texttt{original\_wavs} and used later for plotting, resampling, and spectral preprocessing.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Identify spectral columns}
\CommentTok{\# Spectral variables start with "X" followed by at least three digits.}
\NormalTok{spc\_cols }\OtherTok{\textless{}{-}} \FunctionTok{grep}\NormalTok{(}\StringTok{"\^{}X[0{-}9]\{3,\}"}\NormalTok{, }\FunctionTok{colnames}\NormalTok{(original\_dat))}
\end{Highlighting}
\end{Shaded}

In the above code, the regular expression \texttt{\^{}X{[}0-9{]}\{3,\}} selects columns whose names start with \texttt{X} followed by at least three digits. In this dataset, those columns correspond to MIR absorbance values measured at different wavenumbers. We use that to extract the spectral matrix and convert the column names to numeric wavenumber values:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Extract the spectral matrix}
\NormalTok{spc\_original }\OtherTok{\textless{}{-}}\NormalTok{ original\_dat[, spc\_cols]}

\CommentTok{\# Remove the "X" prefix from spectral column names}
\FunctionTok{colnames}\NormalTok{(spc\_original) }\OtherTok{\textless{}{-}} \FunctionTok{gsub}\NormalTok{(}\StringTok{"\^{}X"}\NormalTok{, }\StringTok{""}\NormalTok{, }\FunctionTok{colnames}\NormalTok{(spc\_original))}

\CommentTok{\# Convert spectral column names to numeric wavenumber values}
\NormalTok{original\_wavs }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{colnames}\NormalTok{(spc\_original))}

\CommentTok{\# Check the spectral range}
\FunctionTok{range}\NormalTok{(original\_wavs)}
\end{Highlighting}
\end{Shaded}
...
Because the spectra are recorded at high spectral resolution, the spectral matrix contains many highly correlated variables. Before modeling, we therefore resample the spectra to a coarser wavenumber grid. This reduces computational demand while preserving the main absorption features. In this case, we resample the spectra to a resolution of 8 cm\(^{-1}\) using \texttt{prospectr::resample()}. Since the original spectra do not extend exactly to 4000 cm\(^{-1}\), we avoid extrapolation (highly recommended for all cases) and resample only up to 3992 cm\(^{-1}\). This results in a new spectral matrix with 425 spectral variables, which is more manageable for modeling while still capturing the key spectral information:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# In case you do not have the prospectr ackage alerady installed}
\CommentTok{\# please run the following line}
\FunctionTok{install.packages}\NormalTok{(}\StringTok{"prospectr"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(prospectr)}
\CommentTok{\# Define a new regular spectral grid for resampling}
\NormalTok{wavs }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{600}\NormalTok{, }\DecValTok{3992}\NormalTok{, }\AttributeTok{by =} \DecValTok{8}\NormalTok{)}
\CommentTok{\# Resample spectra to the new wavelength grid}
\NormalTok{spc\_resampled }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{resample}\NormalTok{(}
\NormalTok{  spc\_original,}
  \AttributeTok{wav =}\NormalTok{ original\_wavs,}
  \AttributeTok{new.wav =}\NormalTok{ wavs}
\NormalTok{)}
\CommentTok{\# check the dimensions}
\FunctionTok{dim}\NormalTok{(spc\_resampled)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# extract non{-}spectral variables into a new dataframe}
\NormalTok{repeated\_dat }\OtherTok{\textless{}{-}}\NormalTok{ original\_dat[, }\SpecialCharTok{{-}}\NormalTok{spc\_cols]}

\CommentTok{\# in the main dataframe create a new variable called spc }
\CommentTok{\# containing the matrix of spectra }
\NormalTok{repeated\_dat}\SpecialCharTok{$}\NormalTok{spc }\OtherTok{\textless{}{-}}\NormalTok{ spc\_resampled}

\CommentTok{\# make some checks}
\FunctionTok{colnames}\NormalTok{(repeated\_dat)}
\FunctionTok{dim}\NormalTok{(repeated\_dat)}
\FunctionTok{dim}\NormalTok{(repeated\_dat}\SpecialCharTok{$}\NormalTok{spc)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Average replicate spectra by sample ID}
\NormalTok{spc\_avg }\OtherTok{\textless{}{-}} \FunctionTok{aggregate}\NormalTok{(}
\NormalTok{  repeated\_dat}\SpecialCharTok{$}\NormalTok{spc,}
  \AttributeTok{by =} \FunctionTok{list}\NormalTok{(}\AttributeTok{smp\_id =}\NormalTok{ repeated\_dat}\SpecialCharTok{$}\NormalTok{smp\_id),}
  \AttributeTok{FUN =}\NormalTok{ mean}
\NormalTok{)}
\CommentTok{\# order the resuslts by sample ID to ensure correct alignment with the main }
\CommentTok{\# dataset later on}
\NormalTok{spc\_avg }\OtherTok{\textless{}{-}}\NormalTok{ spc\_avg[}\FunctionTok{order}\NormalTok{(spc\_avg}\SpecialCharTok{$}\NormalTok{smp\_id),  ]}

\CommentTok{\# aggregate() prepends smp\_id as first column; extract the spectral matrix}
\FunctionTok{rownames}\NormalTok{(spc\_avg) }\OtherTok{\textless{}{-}}\NormalTok{ spc\_avg}\SpecialCharTok{$}\NormalTok{smp\_id}
\NormalTok{spc\_avg }\OtherTok{\textless{}{-}} \FunctionTok{as.matrix}\NormalTok{(spc\_avg[, }\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{])}

\CommentTok{\# Drop duplicate non{-}spectral rows (keep first occurrence per sample)}
\NormalTok{dat }\OtherTok{\textless{}{-}}\NormalTok{ repeated\_dat[}\SpecialCharTok{!}\FunctionTok{duplicated}\NormalTok{(repeated\_dat}\SpecialCharTok{$}\NormalTok{smp\_id), ]}
\NormalTok{dat }\OtherTok{\textless{}{-}}\NormalTok{ dat[}\FunctionTok{order}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{smp\_id), ]}

\CommentTok{\# Verify that sample IDs are aligned before overwriting spectra}
\FunctionTok{stopifnot}\NormalTok{(}\FunctionTok{all}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(spc\_avg) }\SpecialCharTok{==}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{smp\_id))}

\CommentTok{\# Overwrite spectra with the averaged matrix}
\NormalTok{dat}\SpecialCharTok{$}\NormalTok{spc }\OtherTok{\textless{}{-}}\NormalTok{ spc\_avg}

\CommentTok{\# check dimensions of the final dataset}
...
\NormalTok{  wavs,}
  \FunctionTok{t}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{spc),}
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{,}
  \AttributeTok{lty =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{4000}\NormalTok{, }\DecValTok{500}\NormalTok{),}
  \AttributeTok{xlab =} \FunctionTok{expression}\NormalTok{(Wavenumber}\SpecialCharTok{\textasciitilde{}}\NormalTok{(cm}\SpecialCharTok{\^{}}\NormalTok{\{}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{\})),}
  \AttributeTok{ylab =} \StringTok{"Absorbance"}\NormalTok{,}
  \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.8}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{0.3}\NormalTok{)}
\NormalTok{)}
\FunctionTok{grid}\NormalTok{(}\AttributeTok{lty =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-spectra-1} 

}

\caption{Resampled and aggreagted MIR sectra.}\label{fig:fig-spectra}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Create unique profile IDs for each coordinate pair}
\NormalTok{coords }\OtherTok{\textless{}{-}} \FunctionTok{paste}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{Long\_Site.x, dat}\SpecialCharTok{$}\NormalTok{Lat\_Site.x, }\AttributeTok{sep =} \StringTok{"\_"}\NormalTok{)}
\NormalTok{prof\_num }\OtherTok{\textless{}{-}} \FunctionTok{match}\NormalTok{(coords, }\FunctionTok{unique}\NormalTok{(coords))}

\NormalTok{dat}\SpecialCharTok{$}\NormalTok{ProfID }\OtherTok{\textless{}{-}} \FunctionTok{sprintf}\NormalTok{(}\StringTok{"PROF\%04d"}\NormalTok{, prof\_num)}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  \textbf{Smoothing}, such as moving average or Savitzky--Golay filtering, to reduce high-frequency noise.
\item
  \textbf{Scatter correction}, such as standard normal variate (SNV) or multiplicative scatter correction (MSC), to reduce multiplicative effects.
\item
  \textbf{Derivatives}, often combined with smoothing, to reduce baseline effects and enhance spectral features.
\end{itemize}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-first-spectrum-1} 

}

\caption{The first spectrum in the dataset before preprocessing.}\label{fig:fig-first-spectrum}
\end{figure}
...
\CommentTok{\# apply standard normal variate (SNV) correction followed by }
\CommentTok{\# a Savitzky–Golay first derivative}
\NormalTok{a\_spectrum\_first\_der  }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{savitzkyGolay}\NormalTok{(}
\NormalTok{  a\_spectrum, }\CommentTok{\# the spectrum}
  \AttributeTok{m =} \DecValTok{1}\NormalTok{, }\CommentTok{\# the derivative order }
  \AttributeTok{p =} \DecValTok{1}\NormalTok{, }\CommentTok{\# the polynomial order used within the moving window }
  \AttributeTok{w =} \DecValTok{25} \CommentTok{\# the window size in spectral variables}
\NormalTok{)}
\CommentTok{\# the wavenumber values for the first derivative spectrum are trimmed as a }
\CommentTok{\# result of the moving window. We can get the wavenumber values for the first }
\CommentTok{\# derivative spectrum as follows:}
\NormalTok{der\_wavs }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{names}\NormalTok{(a\_spectrum\_first\_der))}
\FunctionTok{plot}\NormalTok{(}
\NormalTok{  der\_wavs,}
\NormalTok{  a\_spectrum\_first\_der,}
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{,}
  \AttributeTok{lty =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{4000}\NormalTok{, }\DecValTok{500}\NormalTok{),}
  \AttributeTok{xlab =} \FunctionTok{expression}\NormalTok{(Wavenumber}\SpecialCharTok{\textasciitilde{}}\NormalTok{(cm}\SpecialCharTok{\^{}}\NormalTok{\{}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{\})),}
  \AttributeTok{ylab =} \StringTok{"First derivative absorbance"}\NormalTok{,}
  \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FloatTok{0.3}\NormalTok{, }\FloatTok{0.8}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# apply standard normal variate (SNV) correction followed by }
\CommentTok{\# a Savitzky–Golay first derivative}
\NormalTok{a\_spectrum\_first\_der\_snv }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{standardNormalVariate}\NormalTok{(}
  \FunctionTok{matrix}\NormalTok{(a\_spectrum\_first\_der, }\AttributeTok{nrow =} \DecValTok{1}\NormalTok{)}
\NormalTok{)}

\FunctionTok{plot}\NormalTok{(}
\NormalTok{  der\_wavs,}
\NormalTok{  a\_spectrum\_first\_der\_snv,}
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{,}
  \AttributeTok{lty =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{4000}\NormalTok{, }\DecValTok{500}\NormalTok{),}
  \AttributeTok{xlab =} \FunctionTok{expression}\NormalTok{(Wavenumber}\SpecialCharTok{\textasciitilde{}}\NormalTok{(cm}\SpecialCharTok{\^{}}\NormalTok{\{}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{\})),}
  \AttributeTok{ylab =} \StringTok{"SNV of first derivative absorbance"}\NormalTok{,}
  \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.8}\NormalTok{, }\FloatTok{0.3}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-first-spectrum-der-snv-1} 

}

\caption{The first spectrum after applying Savitzky–Golay first derivative and standard normal variate (SNV) correction.}\label{fig:fig-first-spectrum-der-snv}
\end{figure}

SNV correction standardizes each spectrum individually. It subtracts the average value of the spectrum and divides by its standard deviation, so the transformed spectrum is centered around zero and expressed on a comparable scale. In Figure (\citeproc{ref-ref}{\textbf{ref?}})(fig:fig-first-spectrum-der-snv), the main shape of the derivative spectrum is preserved, but the vertical scale changes. This is expected: SNV does not create new spectral features; it mainly reduces differences caused by overall signal intensity, sample packing, particle size, or other multiplicative effects. After SNV, the y-axis no longer represents absorbance directly, but standardized derivative values.
...
\CommentTok{\# apply standard normal variate (SNV) correction followed by }
\CommentTok{\# a Savitzky–Golay first derivative}
\NormalTok{dat}\SpecialCharTok{$}\NormalTok{spc\_processed  }\OtherTok{\textless{}{-}}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{spc }\SpecialCharTok{|\textgreater{}} 
\NormalTok{  prospectr}\SpecialCharTok{::}\FunctionTok{savitzkyGolay}\NormalTok{(}\AttributeTok{m =} \DecValTok{1}\NormalTok{, }\AttributeTok{p =} \DecValTok{1}\NormalTok{, }\AttributeTok{w =} \DecValTok{25}\NormalTok{) }\SpecialCharTok{|\textgreater{}} 
\NormalTok{  prospectr}\SpecialCharTok{::}\FunctionTok{standardNormalVariate}\NormalTok{() }

\CommentTok{\# get the wavenumber values for the processed spectra}
\NormalTok{wavs\_processed }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(}\FunctionTok{colnames}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{spc\_processed))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{matplot}\NormalTok{(}
\NormalTok{  wavs\_processed,}
  \FunctionTok{t}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{spc\_processed),}
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{,}
  \AttributeTok{lty =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{xlim =} \FunctionTok{c}\NormalTok{(}\DecValTok{4000}\NormalTok{, }\DecValTok{500}\NormalTok{),}
  \AttributeTok{xlab =} \FunctionTok{expression}\NormalTok{(Wavenumber}\SpecialCharTok{\textasciitilde{}}\NormalTok{(cm}\SpecialCharTok{\^{}}\NormalTok{\{}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{\})),}
  \AttributeTok{ylab =} \StringTok{"SNV of first derivative absorbance"}\NormalTok{, }
  \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.3}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.3}\NormalTok{)}
\NormalTok{)}
\FunctionTok{grid}\NormalTok{(}\AttributeTok{lty =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.8\linewidth]{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-spectra-processed-1} 

}

\caption{Processed MIR spectra after standard normal variate correction and Savitzky–Golay first-derivative transformation.}\label{fig:fig-spectra-processed}
\end{figure}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-cal-sampling-1} 

}

\caption{Example of $k$-means sampling for spectroscopic data. The plot shows the distribution of samples in the space of the first two principal components of the processed spectra. The red points indicate the samples selected for calibration using $k$-means sampling with $k = 85$ cluster centres.}\label{fig:fig-cal-sampling}
\end{figure}

Please see the \href{https://CRAN.R-project.org/package=prospectr}{\texttt{prospectr} package vignettes} for additional examples of calibration sample selection for DRS.
...
\begin{quote}
\emph{The rationale is to simulate a cost-efficient scenario in which conventional laboratory analysis is performed only on a limited, representative subset of samples, while DRS is used to extend soil property information to a larger number of samples through model-based prediction.}
\end{quote}

Here, approximately 20\% of the dataset is used for calibration. The calibration subset was selected using the \(k\)-means sampling algorithm for spectroscopic data (\citeproc{ref-naes1987design}{Næs, 1987}), implemented in the \texttt{naes()} function of the \texttt{prospectr} package. This method partitions the spectral space into \(k\) clusters and selects the sample closest to each cluster centroid, ensuring that the selected subset is well distributed across the main axes of spectral variation. Such a strategy has been shown to provide representative calibration sets that reduce generalization error (\citeproc{ref-ramirez2014sampling}{Ramirez-Lopez \emph{et al.}, 2014}).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Target number of cluster centres}
\NormalTok{n\_cal }\OtherTok{\textless{}{-}} \DecValTok{85}

\CommentTok{\# Select spectrally representative samples using k{-}means sampling}
\CommentTok{\# in the space of the first 10 principal components}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{201909}\NormalTok{)}
\NormalTok{kms }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{naes}\NormalTok{(}
\NormalTok{  dat}\SpecialCharTok{$}\NormalTok{spc\_processed,}
  \AttributeTok{k =}\NormalTok{ n\_cal,}
  \AttributeTok{pc =} \DecValTok{10}\NormalTok{,}
  \AttributeTok{iter.max =} \DecValTok{1000}
\NormalTok{)}

\CommentTok{\# Expand the selection to include all horizon samples from the profiles}
\CommentTok{\# of the selected samples. This avoids splitting a soil profile between}
\CommentTok{\# calibration and prediction subsets, which would constitute information}
\CommentTok{\# leakage. Profiles typically contain multiple horizons, so the final}
\CommentTok{\# calibration set will be larger than n\_cal.}
\NormalTok{kms\_samples }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%} \FunctionTok{unique}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID[kms}\SpecialCharTok{$}\NormalTok{model]))}
\end{Highlighting}
\end{Shaded}

The \texttt{kms\_samples} vector contains the indices of the samples selected for model calibration. The remaining samples are treated as if their SOC content were unknown and are predicted using the calibrated DRS model. So we split the data into calibration and prediction subsets:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat\_cal }\OtherTok{\textless{}{-}}\NormalTok{ dat[kms\_samples, ]}
\NormalTok{dat\_pred }\OtherTok{\textless{}{-}}\NormalTok{ dat[}\SpecialCharTok{{-}}\NormalTok{kms\_samples, ]}
\end{Highlighting}
\end{Shaded}
...
For the RF modeling step, we use the \texttt{ranger} package (\citeproc{ref-wright2017ranger}{Wright and Ziegler, 2017}).

Prediction uncertainty is quantified by the model using the conditional distribution of the predictions. This approach is described in detail for soil DRS data by Wadoux and Ramirez-Lopez (\citeproc{ref-wadoux2025uncertainty}{2025}). From this model distribution, a mean predicted value, a prediction variance, and a prediction standard deviation are calculated for each soil sample. These quantities represent the required outputs of the DRS modeling step for DSM.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# set.seed() makes the random assignment reproducible: rerunning the}
\CommentTok{\# script always produces the same fold structure. The value 123 is arbitrary.}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{14092019}\NormalTok{) }\CommentTok{\# just any number}

\CommentTok{\# 10 folds}
\NormalTok{nfolds }\OtherTok{\textless{}{-}} \DecValTok{10}
\CommentTok{\# We assign folds at the profile level because observations within a}
\CommentTok{\# profile are depth{-}wise measurements of the same soil column... they}
\CommentTok{\# are not independent. Splitting at the observation level would leak}
\CommentTok{\# information from the same profile into both training and test sets.}
\NormalTok{cal\_profile\_ids }\OtherTok{\textless{}{-}} \FunctionTok{unique}\NormalTok{(dat\_cal}\SpecialCharTok{$}\NormalTok{ProfID)}
\NormalTok{fold\_index  }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\NormalTok{nfolds, }\AttributeTok{length.out =} \FunctionTok{length}\NormalTok{(cal\_profile\_ids)))}

\CommentTok{\# Each observation inherits the fold of its parent profile}
\NormalTok{dat\_cal}\SpecialCharTok{$}\NormalTok{fold }\OtherTok{\textless{}{-}}\NormalTok{ fold\_index[}\FunctionTok{match}\NormalTok{(dat\_cal}\SpecialCharTok{$}\NormalTok{ProfID, cal\_profile\_ids)]}
\end{Highlighting}
\end{Shaded}

Now we can proceed with the modeling. In this implementation for each fold we use the option \texttt{quantreg\ =\ TRUE} which enables estimation of the conditional distribution of SOC rather than only a single point prediction. The forest was grown with 3500 trees, bootstrap sampling with replacement, and a minimum node size of 10. Splits were selected using the maximally selected rank statistic (\texttt{splitrule\ =\ "maxstat"}), and a fixed seed was used for reproducibility. For each held-out sample, 100 values were drawn from the estimated conditional distribution and summarised by their mean, variance, and standard deviation. Further details on the \texttt{ranger} implementation and this uncertainty-aware modelling approach are provided by Wright and Ziegler (\citeproc{ref-wright2017ranger}{2017}) and Wadoux and Ramirez-Lopez (\citeproc{ref-wadoux2025uncertainty}{2025}).

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the ranger package for quantile forest modeling }
\FunctionTok{library}\NormalTok{(ranger)}
\FunctionTok{library}\NormalTok{(matrixStats)}

\CommentTok{\# Prepare storage vectors for CV predictions and uncertainty}
\NormalTok{SOC\_predRF }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA\_real\_}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(dat\_cal))  }\CommentTok{\# mean prediction}
\NormalTok{SOC\_varRF }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA\_real\_}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(dat\_cal))  }\CommentTok{\# predictive variance}
\NormalTok{SOC\_sdRF }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA\_real\_}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(dat\_cal))  }\CommentTok{\# predictive standard deviation}

\CommentTok{\# Run 10{-}fold cross{-}validation}
\ControlFlowTok{for}\NormalTok{ (k }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(nfolds)) \{}
  \FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}r}\StringTok{Processing fold"}\NormalTok{, k, }\StringTok{"out of 10"}\NormalTok{)}
  \CommentTok{\# Test set: all samples belonging to fold k}
\NormalTok{  test\_rows }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(dat\_cal}\SpecialCharTok{$}\NormalTok{fold }\SpecialCharTok{==}\NormalTok{ k)}
  \CommentTok{\# Training set: all remaining samples}
\NormalTok{  train\_rows }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(dat\_cal}\SpecialCharTok{$}\NormalTok{fold }\SpecialCharTok{!=}\NormalTok{ k)}
  \CommentTok{\# Fit quantile random forest on training data}
\NormalTok{  model }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
    \AttributeTok{x =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{spc\_processed[train\_rows, ],}
    \AttributeTok{y =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC[train\_rows],}
    \AttributeTok{quantreg =} \ConstantTok{TRUE}\NormalTok{,}
    \AttributeTok{num.trees =} \DecValTok{3500}\NormalTok{,}
    \AttributeTok{sample.fraction =} \DecValTok{1}\NormalTok{,}
    \AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{,}
    \AttributeTok{splitrule =} \StringTok{"maxstat"}\NormalTok{,}
    \AttributeTok{min.node.size =} \DecValTok{10}\NormalTok{, }
    \AttributeTok{seed =} \DecValTok{201909}
\NormalTok{  )}
...
\NormalTok{    model,}
    \AttributeTok{data =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{spc\_processed[test\_rows, ],}
    \AttributeTok{type =} \StringTok{"quantiles"}\NormalTok{,}
...
\NormalTok{dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF }\OtherTok{\textless{}{-}}\NormalTok{ SOC\_predRF}
\NormalTok{dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_varRF }\OtherTok{\textless{}{-}}\NormalTok{ SOC\_varRF}
\NormalTok{dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_sdRF }\OtherTok{\textless{}{-}}\NormalTok{ SOC\_sdRF}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{xy\_lims }\OtherTok{\textless{}{-}} \FunctionTok{range}\NormalTok{(dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF, dat\_cal}\SpecialCharTok{$}\NormalTok{SOC)}
\CommentTok{\# Validation plot}
\FunctionTok{plot}\NormalTok{(}
  \AttributeTok{x =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF,}
  \AttributeTok{y =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC,}
  \AttributeTok{xlab =} \StringTok{"Predicted SOC (\%)"}\NormalTok{,}
  \AttributeTok{ylab =} \StringTok{"Measured SOC (\%)"}\NormalTok{,}
  \AttributeTok{xlim =}\NormalTok{ xy\_lims,}
  \AttributeTok{ylim =}\NormalTok{ xy\_lims,}
...
\CommentTok{\# add the error bars representing the prediction standard deviation}
\FunctionTok{arrows}\NormalTok{(}
  \AttributeTok{x0 =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF }\SpecialCharTok{{-}}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_sdRF,}
  \AttributeTok{y0 =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC,}
  \AttributeTok{x1 =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF }\SpecialCharTok{+}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_sdRF,}
  \AttributeTok{y1 =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC,}
  \AttributeTok{code =} \DecValTok{3}\NormalTok{,}
  \AttributeTok{length =} \DecValTok{0}\NormalTok{,}
  \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{0.5}\NormalTok{)}
\NormalTok{)}

\FunctionTok{abline}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\AttributeTok{col =}\NormalTok{ col\_ref, }\AttributeTok{lwd =} \FloatTok{1.5}\NormalTok{, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\FunctionTok{grid}\NormalTok{(}\AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.6}\NormalTok{), }\AttributeTok{lty =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-val-plot-1} 

}

\caption{Validation plot of predicted SOC values against laboratory reference measurements. Error bars represent the prediction standard deviation for each sample. The dashed line indicates the 1:1 relationship, which represents perfect agreement between predictions and measurements.}\label{fig:fig-val-plot}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{drs\_rmse }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{mean}\NormalTok{((dat\_cal}\SpecialCharTok{$}\NormalTok{SOC\_predRF }\SpecialCharTok{{-}}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{))}
\FunctionTok{cat}\NormalTok{(}\StringTok{"DRS model RMSE:"}\NormalTok{, }\FunctionTok{round}\NormalTok{(drs\_rmse, }\DecValTok{3}\NormalTok{), }\StringTok{"\% SOC}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{final\_drs\_model }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
  \AttributeTok{x =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{spc\_processed,}
  \AttributeTok{y =}\NormalTok{ dat\_cal}\SpecialCharTok{$}\NormalTok{SOC,}
  \AttributeTok{quantreg =} \ConstantTok{TRUE}\NormalTok{,}
  \AttributeTok{num.trees =} \DecValTok{3500}\NormalTok{,}
  \AttributeTok{sample.fraction =} \DecValTok{1}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Predict for the remaining samples using the final model}
\NormalTok{drs\_soc\_preds }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(}
\NormalTok{  final\_drs\_model,}
  \AttributeTok{data =}\NormalTok{ dat\_pred}\SpecialCharTok{$}\NormalTok{spc\_processed,}
  \AttributeTok{type =} \StringTok{"quantiles"}\NormalTok{,}
  \AttributeTok{what =} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{sample}\NormalTok{(x, }\DecValTok{100}\NormalTok{, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{)}\SpecialCharTok{$}\NormalTok{predictions}
  
\CommentTok{\# Store prediction summary statistics}
\NormalTok{dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_predRF }\OtherTok{\textless{}{-}} \FunctionTok{rowMeans}\NormalTok{(drs\_soc\_preds)}
\NormalTok{dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_varRF }\OtherTok{\textless{}{-}} \FunctionTok{rowVars}\NormalTok{(drs\_soc\_preds)}
\NormalTok{dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_sdRF }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_varRF)}
\end{Highlighting}
\end{Shaded}
...
\begin{itemize}
\item
  \textbf{Laboratory SOC measurements} for the samples used to calibrate the DRS model. These samples represent approximately 20\% of the original dataset. Because these values are assumed to be available from conventional laboratory analysis, they are retained in preference to DRS-based predictions.
\item
  \textbf{DRS-based SOC predictions} for the remaining samples, which were not used in the calibration of the DRS model. As described above, these samples are treated as if laboratory SOC values were unavailable. Their SOC values are therefore estimated entirely from the DRS model. This setup simulates a realistic application scenario in which DRS is used to increase the number of SOC observations available for DSM.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Extract metadata for the augmented dataset}
\NormalTok{dat\_augmented }\OtherTok{\textless{}{-}}\NormalTok{ dat[, }\FunctionTok{c}\NormalTok{(}
  \StringTok{"smp\_id"}\NormalTok{, }\StringTok{"ProfID"}\NormalTok{, }\StringTok{"Long\_Site.x"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{, }
  \StringTok{"Top\_depth\_cm.x"}\NormalTok{, }\StringTok{"Bottom\_depth\_cm.x"}
\NormalTok{  )}
\NormalTok{]}

\CommentTok{\# Initialise SOC values and associated standard deviations}
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC }\OtherTok{\textless{}{-}} \ConstantTok{NA}
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC\_sd }\OtherTok{\textless{}{-}} \ConstantTok{NA}

\CommentTok{\# Add laboratory SOC values for the samples used to calibrate the DRS model}
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC[kms\_samples] }\OtherTok{\textless{}{-}}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{SOC[kms\_samples]}

\CommentTok{\# Add DRS{-}based SOC predictions for the remaining samples}
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC[}\SpecialCharTok{{-}}\NormalTok{kms\_samples] }\OtherTok{\textless{}{-}}\NormalTok{ dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_predRF }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC\_sd[}\SpecialCharTok{{-}}\NormalTok{kms\_samples] }\OtherTok{\textless{}{-}}\NormalTok{ dat\_pred}\SpecialCharTok{$}\NormalTok{SOC\_sdRF }
\end{Highlighting}
\end{Shaded}
...
Quantifying laboratory measurement uncertainty in the absence of replicate measurements requires an external reference. Stevens \emph{et al.} (\citeproc{ref-stevens2013prediction}{2013}) assessed the reproducibility of dry combustion SOC analysis across a large harmonised European soil survey and reported a standard error of laboratory (SEL) of approximately 2 g C kg⁻¹ for mineral soils overall (equivalent to 0.2\% SOC). This value reflects intermediate precision under standardised conditions in an accredited laboratory and represents a realistic but optimistic estimate of the analytical component of uncertainty, excluding sampling and preparation variability.

For this dataset, which was similarly analysed under standardised laboratory conditions (\citeproc{ref-dangal2019accurate}{Dangal \emph{et al.}, 2019}), we adopt a constant absolute uncertainty of \(\sigma_{\text{lab}} = 0.15\)\% SOC as a pragmatic approximation consistent with the published literature. A constant rather than proportional error model is used here because a proportional formulation (where \(\sigma = CV \times y_i\)) causes low-SOC observations to receive disproportionately high weights during model calibration, irrespective of their actual reliability. In practical applications, laboratory-specific replicate measurements should be used to derive empirical uncertainty estimates.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Constant absolute laboratory uncertainty: 0.15\% SOC}
\CommentTok{\# Based on Stevens et al. (2013), SEL ≈ 2 g C kg{-}1 for mineral soils}
\NormalTok{dat}\SpecialCharTok{$}\NormalTok{SOC\_sd }\OtherTok{\textless{}{-}} \FloatTok{0.15}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat\_augmented}\SpecialCharTok{$}\NormalTok{SOC\_sd[kms\_samples] }\OtherTok{\textless{}{-}} \FloatTok{0.15}
\end{Highlighting}
\end{Shaded}
...
We first load the \texttt{aqp} package and convert both datasets to \texttt{SoilProfileCollection} objects, which store horizon-level data in a structured format that supports depth-based operations:
...
\FunctionTok{depths}\NormalTok{(dat\_augmented) }\OtherTok{\textless{}{-}}\NormalTok{ ProfID }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Top\_depth\_cm.x }\SpecialCharTok{+}\NormalTok{ Bottom\_depth\_cm.x}
\end{Highlighting}
\end{Shaded}

Combining uncertainty estimates across horizons requires variance propagation. For a depth-weighted average \(\hat{y} = \sum_i w_i y_i / \sum_i w_i\), where \(w_i\) is the thickness of horizon \(i\) within the target interval, the aggregated standard deviation is \(\sigma = \sqrt{\sum_i w_i^2 \sigma_i^2} / \sum_i w_i\). The helper function below implements this for an arbitrary depth interval, returning \texttt{NA} for profiles that do not fully cover it:
...
\NormalTok{  sd   }\OtherTok{\textless{}{-}}\NormalTok{ sd[keep]}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{sum}\NormalTok{(w) }\SpecialCharTok{\textless{}}\NormalTok{ (z2 }\SpecialCharTok{{-}}\NormalTok{ z1)) }\FunctionTok{return}\NormalTok{(}\ConstantTok{NA}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{soc\_mean\_ref }\OtherTok{\textless{}{-}} \FunctionTok{profileApply}\NormalTok{(dat, }\ControlFlowTok{function}\NormalTok{(p) \{}
\NormalTok{  h }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(p)}
\NormalTok{  w }\OtherTok{\textless{}{-}} \FunctionTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FunctionTok{pmin}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Bottom\_depth\_cm.x, }\DecValTok{30}\NormalTok{) }\SpecialCharTok{{-}} \FunctionTok{pmax}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Top\_depth\_cm.x, }\DecValTok{0}\NormalTok{))}
\NormalTok{  keep }\OtherTok{\textless{}{-}}\NormalTok{ w }\SpecialCharTok{\textgreater{}} \DecValTok{0} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{SOC)}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{sum}\NormalTok{(w[keep]) }\SpecialCharTok{\textless{}} \DecValTok{30}\NormalTok{) }\FunctionTok{return}\NormalTok{(}\ConstantTok{NA}\NormalTok{)}
  \FunctionTok{sum}\NormalTok{(w[keep] }\SpecialCharTok{*}\NormalTok{ h}\SpecialCharTok{$}\NormalTok{SOC[keep]) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(w[keep])}
\NormalTok{\})}

\NormalTok{soc\_sd\_ref }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(dat) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{group\_by}\NormalTok{(ProfID) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{summarise}\NormalTok{(}
    \AttributeTok{SOC\_sd\_0\_30 =} \FunctionTok{weighted\_sd\_0\_30}\NormalTok{(Top\_depth\_cm.x, Bottom\_depth\_cm.x, SOC\_sd),}
    \AttributeTok{.groups =} \StringTok{"drop"}
\NormalTok{  )}

\NormalTok{coords\_ref }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(dat)[, }\FunctionTok{c}\NormalTok{(}\StringTok{"ProfID"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{, }\StringTok{"Long\_Site.x"}\NormalTok{)]}
\NormalTok{coords\_ref }\OtherTok{\textless{}{-}}\NormalTok{ coords\_ref[}\SpecialCharTok{!}\FunctionTok{duplicated}\NormalTok{(coords\_ref}\SpecialCharTok{$}\NormalTok{ProfID), ]}

\NormalTok{soc\_ref\_0\_30\_xy }\OtherTok{\textless{}{-}}\NormalTok{ soc\_sd\_ref }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{SOC\_0\_30 =}\NormalTok{ soc\_mean\_ref) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{select}\NormalTok{(ProfID, SOC\_0\_30, SOC\_sd\_0\_30) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{merge}\NormalTok{(coords\_ref, }\AttributeTok{by =} \StringTok{"ProfID"}\NormalTok{, }\AttributeTok{all.x =} \ConstantTok{TRUE}\NormalTok{)}

\FunctionTok{head}\NormalTok{(soc\_ref\_0\_30\_xy)}
\end{Highlighting}
\end{Shaded}
...
The augmented dataset combines laboratory measurements with DRS-based predictions. The single \texttt{SOC\_sd} column carries the appropriate uncertainty for each observation (laboratory analytical error for reference samples, RF prediction uncertainty for DRS-predicted samples) so the same function applies without modification:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{soc\_mean\_aug }\OtherTok{\textless{}{-}} \FunctionTok{profileApply}\NormalTok{(}
\NormalTok{  dat\_augmented, }
  \ControlFlowTok{function}\NormalTok{(p) \{}
\NormalTok{    h }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(p)}
\NormalTok{    w }\OtherTok{\textless{}{-}} \FunctionTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FunctionTok{pmin}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Bottom\_depth\_cm.x, }\DecValTok{30}\NormalTok{) }\SpecialCharTok{{-}} \FunctionTok{pmax}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Top\_depth\_cm.x, }\DecValTok{0}\NormalTok{))}
\NormalTok{    keep }\OtherTok{\textless{}{-}}\NormalTok{ w }\SpecialCharTok{\textgreater{}} \DecValTok{0} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{SOC)}
    \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{sum}\NormalTok{(w[keep]) }\SpecialCharTok{\textless{}} \DecValTok{30}\NormalTok{) }
      \FunctionTok{return}\NormalTok{(}\ConstantTok{NA}\NormalTok{)}
    \FunctionTok{sum}\NormalTok{(w[keep] }\SpecialCharTok{*}\NormalTok{ h}\SpecialCharTok{$}\NormalTok{SOC[keep]) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(w[keep])}
\NormalTok{  \}}
\NormalTok{)}

\NormalTok{soc\_sd\_aug }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(dat\_augmented) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{group\_by}\NormalTok{(ProfID) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{summarise}\NormalTok{(}
    \AttributeTok{SOC\_sd\_0\_30 =} \FunctionTok{weighted\_sd\_0\_30}\NormalTok{(Top\_depth\_cm.x, Bottom\_depth\_cm.x, SOC\_sd),}
...
\NormalTok{coords\_aug }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(dat\_augmented)[, }\FunctionTok{c}\NormalTok{(}\StringTok{"ProfID"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{, }\StringTok{"Long\_Site.x"}\NormalTok{)]}
\NormalTok{coords\_aug }\OtherTok{\textless{}{-}}\NormalTok{ coords\_aug[}\SpecialCharTok{!}\FunctionTok{duplicated}\NormalTok{(coords\_aug}\SpecialCharTok{$}\NormalTok{ProfID), ]}

\NormalTok{soc\_aug\_0\_30\_xy }\OtherTok{\textless{}{-}}\NormalTok{ soc\_sd\_aug }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{SOC\_0\_30 =}\NormalTok{ soc\_mean\_aug) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{select}\NormalTok{(ProfID, SOC\_0\_30, SOC\_sd\_0\_30) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{merge}\NormalTok{(coords\_aug, }\AttributeTok{by =} \StringTok{"ProfID"}\NormalTok{, }\AttributeTok{all.x =} \ConstantTok{TRUE}\NormalTok{)}

\FunctionTok{head}\NormalTok{(soc\_aug\_0\_30\_xy)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dens\_ref }\OtherTok{\textless{}{-}} \FunctionTok{density}\NormalTok{(soc\_ref\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_0\_30, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{dens\_aug }\OtherTok{\textless{}{-}} \FunctionTok{density}\NormalTok{(soc\_aug\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_0\_30, }\AttributeTok{bw =}\NormalTok{ dens\_ref}\SpecialCharTok{$}\NormalTok{bw, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}

\FunctionTok{plot}\NormalTok{(}
\NormalTok{  dens\_ref}\SpecialCharTok{$}\NormalTok{x, }
\NormalTok{  dens\_ref}\SpecialCharTok{$}\NormalTok{y, }
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{, }
  \AttributeTok{col =} \StringTok{"\#4E84A8"}\NormalTok{, }
  \AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }
  \AttributeTok{ylab =} \StringTok{"Density"}\NormalTok{,}
  \AttributeTok{xlab =} \StringTok{"SOC, \%"}
\NormalTok{)}
\FunctionTok{lines}\NormalTok{(}
\NormalTok{  dens\_aug}\SpecialCharTok{$}\NormalTok{x, }
\NormalTok{  dens\_aug}\SpecialCharTok{$}\NormalTok{y, }
  \AttributeTok{col =} \StringTok{"\#C1614A"}\NormalTok{, }
  \AttributeTok{lwd =} \DecValTok{2}
\NormalTok{)}
\FunctionTok{legend}\NormalTok{(}
  \StringTok{"topright"}\NormalTok{,}
  \AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Conventional lab"}\NormalTok{, }\StringTok{"DRS{-}augmented"}\NormalTok{),}
  \AttributeTok{col    =} \FunctionTok{c}\NormalTok{(}\StringTok{"\#4E84A8"}\NormalTok{, }\StringTok{"\#C1614A"}\NormalTok{),}
  \AttributeTok{lwd    =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{bty    =} \StringTok{"n"}
\NormalTok{)}
\end{Highlighting}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-soc-distributions-1} 

}

\caption{Empirical distributions of depth-harmonised SOC (0–30 cm) for the reference dataset (conventional laboratory measurements only) and the augmented dataset (laboratory measurements combined with DRS-based predictions).}\label{fig:fig-soc-distributions}
\end{figure}
...
\begin{Highlighting}[]
\CommentTok{\# Based on Stevens et al. (2013), SEL ≈ 2 g C kg{-}1 for mineral soils}
\NormalTok{soc\_ref\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30 }\OtherTok{\textless{}{-}} \FloatTok{0.15}
\NormalTok{soc\_aug\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30[}
\NormalTok{  soc\_aug\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%} \FunctionTok{unique}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID[kms\_samples])}
\NormalTok{] }\OtherTok{\textless{}{-}} \FloatTok{0.15}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{covs }\OtherTok{\textless{}{-}} \FunctionTok{rast}\NormalTok{(}
  \StringTok{"../SoilFER{-}Training{-}Resources/01\_data/module1/training\_data/Environmental\_Covariates\_250m\_KANSAS.tif"}
\NormalTok{)}
\NormalTok{cov\_names }\OtherTok{\textless{}{-}} \FunctionTok{names}\NormalTok{(covs)}

\CommentTok{\# Reference dataset (laboratory measurements only)}
\NormalTok{soil\_pts\_ref }\OtherTok{\textless{}{-}} \FunctionTok{vect}\NormalTok{(}
\NormalTok{  soc\_ref\_0\_30\_xy,}
...
\NormalTok{)}
\NormalTok{soil\_pts\_ref }\OtherTok{\textless{}{-}} \FunctionTok{project}\NormalTok{(soil\_pts\_ref, covs)}
\NormalTok{soil\_pts\_ref }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(soil\_pts\_ref, terra}\SpecialCharTok{::}\FunctionTok{extract}\NormalTok{(covs, soil\_pts\_ref)[, }\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{])}

\CommentTok{\# Augmented dataset (laboratory + DRS{-}based predictions)}
\NormalTok{soil\_pts\_aug }\OtherTok{\textless{}{-}} \FunctionTok{vect}\NormalTok{(}
\NormalTok{  soc\_aug\_0\_30\_xy,}
  \AttributeTok{geom =} \FunctionTok{c}\NormalTok{(}\StringTok{"Long\_Site.x"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{),}
  \AttributeTok{crs =} \StringTok{"EPSG:4326"}
\NormalTok{)}
\NormalTok{soil\_pts\_aug }\OtherTok{\textless{}{-}} \FunctionTok{project}\NormalTok{(soil\_pts\_aug, covs)}
\NormalTok{soil\_pts\_aug }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(soil\_pts\_aug, terra}\SpecialCharTok{::}\FunctionTok{extract}\NormalTok{(covs, soil\_pts\_aug)[, }\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{plot}\NormalTok{(soil\_pts\_ref, }\AttributeTok{cex =} \FloatTok{0.7}\NormalTok{, }\AttributeTok{col =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
In the following example, we again use \emph{quantile regression forests}, this time to map SOC following a workflow similar to the previous DSM examples. The workflow includes deriving uncertainty-based weights, fitting models with and without spectroscopic data, and comparing the resulting spatial predictions.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Compute inverse{-}variance weights}
\NormalTok{soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{/}\NormalTok{ soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{/}\NormalTok{ soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30}\SpecialCharTok{\^{}}\DecValTok{2}

\CommentTok{\# Normalise by the maximum weight within each dataset}
\NormalTok{soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}}\NormalTok{ soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{weight }\SpecialCharTok{/} \FunctionTok{max}\NormalTok{(soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{weight, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}}\NormalTok{ soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{weight }\SpecialCharTok{/} \FunctionTok{max}\NormalTok{(soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{weight, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}

\FunctionTok{summary}\NormalTok{(soil\_pts\_ref}\SpecialCharTok{$}\NormalTok{weight)}
\FunctionTok{summary}\NormalTok{(soil\_pts\_aug}\SpecialCharTok{$}\NormalTok{weight)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# Prepare modeling table}
\NormalTok{soil\_df\_ref }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(soil\_pts\_ref, }\AttributeTok{geom =} \StringTok{"XY"}\NormalTok{)[ ,}
  \FunctionTok{c}\NormalTok{(}\StringTok{"x"}\NormalTok{, }\StringTok{"y"}\NormalTok{, }\StringTok{"SOC\_0\_30"}\NormalTok{, }\StringTok{"SOC\_sd\_0\_30"}\NormalTok{, }\StringTok{"weight"}\NormalTok{, cov\_names)}
\NormalTok{]}
\NormalTok{soil\_df\_ref }\OtherTok{\textless{}{-}}\NormalTok{ soil\_df\_ref[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(soil\_df\_ref}\SpecialCharTok{$}\NormalTok{SOC\_0\_30), ]}

\CommentTok{\# Fit baseline random forest model using laboratory SOC}
\NormalTok{form }\OtherTok{\textless{}{-}} \FunctionTok{as.formula}\NormalTok{(}
  \FunctionTok{paste}\NormalTok{(}\StringTok{"SOC\_0\_30 \textasciitilde{}"}\NormalTok{, }\FunctionTok{paste}\NormalTok{(cov\_names, }\AttributeTok{collapse =} \StringTok{" + "}\NormalTok{))}
\NormalTok{)}

\NormalTok{mod\_ref }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
  \AttributeTok{formula =}\NormalTok{ form,}
  \AttributeTok{data =}\NormalTok{ soil\_df\_ref,}
  \AttributeTok{case.weights =}\NormalTok{ soil\_df\_ref}\SpecialCharTok{$}\NormalTok{weight,}
  \AttributeTok{replace =} \ConstantTok{FALSE}\NormalTok{,}
  \AttributeTok{sample.fraction =} \FloatTok{0.632}\NormalTok{,}
  \AttributeTok{num.trees =} \DecValTok{500}\NormalTok{,}
  \AttributeTok{seed =} \DecValTok{201909}
\NormalTok{)}

\CommentTok{\# Predict SOC across the covariate raster stack}
\NormalTok{soc\_map\_ref }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(covs, mod\_ref, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Plot baseline SOC map}
\FunctionTok{plot}\NormalTok{(soc\_map\_ref, }\AttributeTok{main =} \StringTok{"Baseline SOC map (laboratory observations only)"}\NormalTok{, }\AttributeTok{range =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\subsubsection{DSM model using DRS-augmented data}\label{dsm-model-using-drs-augmented-data}

The augmented dataset combines laboratory SOC measurements with DRS-based predictions, increasing the number of observations available for model calibration. The modeling procedure follows the same structure as the baseline, with one difference: observation weights derived from the unified \texttt{SOC\_sd} column are passed to the model, so that laboratory measurements and DRS predictions contribute proportionally to their respective uncertainties. The resulting SOC map is shown in Figure \ref{fig:fig-mod-augmented} and was generated using the following code:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Prepare modeling table}
\NormalTok{soil\_df\_aug }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(soil\_pts\_aug, }\AttributeTok{geom =} \StringTok{"XY"}\NormalTok{)[,}
  \FunctionTok{c}\NormalTok{(}\StringTok{"x"}\NormalTok{, }\StringTok{"y"}\NormalTok{, }\StringTok{"SOC\_0\_30"}\NormalTok{, }\StringTok{"SOC\_sd\_0\_30"}\NormalTok{, }\StringTok{"weight"}\NormalTok{, cov\_names)}
\NormalTok{]}

\CommentTok{\# Remove profiles with missing SOC}
\NormalTok{soil\_df\_aug }\OtherTok{\textless{}{-}}\NormalTok{ soil\_df\_aug[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(soil\_df\_aug}\SpecialCharTok{$}\NormalTok{SOC\_0\_30), ]}

\CommentTok{\# Fit random forest model using the augmented dataset}
\NormalTok{mod\_aug }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
  \AttributeTok{formula =}\NormalTok{ form,}
  \AttributeTok{data =}\NormalTok{ soil\_df\_aug,}
  \AttributeTok{case.weights =}\NormalTok{ soil\_df\_aug}\SpecialCharTok{$}\NormalTok{weight,}
  \AttributeTok{replace =} \ConstantTok{FALSE}\NormalTok{,}
  \AttributeTok{sample.fraction =} \FloatTok{0.632}\NormalTok{,}
  \AttributeTok{num.trees =} \DecValTok{500}\NormalTok{,}
  \AttributeTok{seed =} \DecValTok{201909}
...
\CommentTok{\# Predict SOC across the covariate raster stack}
\NormalTok{soc\_map\_aug }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(covs, mod\_aug, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Plot augmented SOC map}
\FunctionTok{plot}\NormalTok{(soc\_map\_aug, }\AttributeTok{main =} \StringTok{"SOC map (DRS{-}augmented dataset)"}\NormalTok{, }\AttributeTok{range =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diff\_map }\OtherTok{\textless{}{-}}\NormalTok{ soc\_map\_aug }\SpecialCharTok{{-}}\NormalTok{ soc\_map\_ref}

\FunctionTok{plot}\NormalTok{(}
\NormalTok{  diff\_map,}
  \AttributeTok{main   =} \StringTok{"Difference (augmented {-} baseline)"}\NormalTok{,}
  \AttributeTok{col    =} \FunctionTok{rev}\NormalTok{(}\FunctionTok{hcl.colors}\NormalTok{(}\DecValTok{100}\NormalTok{, }\StringTok{"RdBu"}\NormalTok{)),}
  \AttributeTok{range =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{1.5}\NormalTok{, }\FloatTok{1.5}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-dsm-ref-aug-1} 

}

\caption{SOC predictions from the baseline model (top) and the DRS-augmented model (middle), and their difference (augmented minus baseline, bottom).}\label{fig:fig-dsm-ref-aug}
\end{figure}
...
\NormalTok{wME }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(obs, pred, w) \{}
  \FunctionTok{sum}\NormalTok{(w }\SpecialCharTok{*}\NormalTok{ (pred }\SpecialCharTok{{-}}\NormalTok{ obs), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(w, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{\}}

\NormalTok{wRMSE }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(obs, pred, w) \{}
  \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{sum}\NormalTok{(w }\SpecialCharTok{*}\NormalTok{ (pred }\SpecialCharTok{{-}}\NormalTok{ obs)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(w, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{))}
\NormalTok{\}}

\NormalTok{wNSE }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(obs, pred, w) \{}
\NormalTok{  zbar\_w }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(w }\SpecialCharTok{*}\NormalTok{ obs, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{/} \FunctionTok{sum}\NormalTok{(w, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{run\_cv }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(data, response, predictors, }\AttributeTok{nfolds =} \DecValTok{10}\NormalTok{, }\AttributeTok{seed =} \DecValTok{201909}\NormalTok{) \{}
  \FunctionTok{set.seed}\NormalTok{(seed)}
\NormalTok{  n }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(data)}
\NormalTok{  fold\_id }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\FunctionTok{seq\_len}\NormalTok{(nfolds), }\AttributeTok{length.out =}\NormalTok{ n))}
\NormalTok{  cv\_pred }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA\_real\_}\NormalTok{, n)}
\NormalTok{  form }\OtherTok{\textless{}{-}} \FunctionTok{as.formula}\NormalTok{(}\FunctionTok{paste}\NormalTok{(response, }\StringTok{"\textasciitilde{}"}\NormalTok{, }\FunctionTok{paste}\NormalTok{(predictors, }\AttributeTok{collapse =} \StringTok{" + "}\NormalTok{)))}
  
  \ControlFlowTok{for}\NormalTok{ (fold }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(nfolds)) \{}
\NormalTok{    train }\OtherTok{\textless{}{-}}\NormalTok{ data[fold\_id }\SpecialCharTok{!=}\NormalTok{ fold, ]}
\NormalTok{    test }\OtherTok{\textless{}{-}}\NormalTok{ data[fold\_id }\SpecialCharTok{==}\NormalTok{ fold, ]}
\NormalTok{    mod }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
      \AttributeTok{formula =}\NormalTok{ form,}
      \AttributeTok{data =}\NormalTok{ train,}
      \AttributeTok{case.weights =}\NormalTok{ train}\SpecialCharTok{$}\NormalTok{weight,}
      \AttributeTok{replace =} \ConstantTok{FALSE}\NormalTok{,}
      \AttributeTok{sample.fraction =} \FloatTok{0.632}\NormalTok{,}
      \AttributeTok{seed =}\NormalTok{ seed}
\NormalTok{    )}
\NormalTok{    cv\_pred[fold\_id }\SpecialCharTok{==}\NormalTok{ fold] }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(mod, }\AttributeTok{data =}\NormalTok{ test)}\SpecialCharTok{$}\NormalTok{predictions}
\NormalTok{  \}}
  
  \FunctionTok{data.frame}\NormalTok{(}
    \AttributeTok{obs =}\NormalTok{ data[[response]],}
    \AttributeTok{pred =}\NormalTok{ cv\_pred,}
    \AttributeTok{weight =}\NormalTok{ data}\SpecialCharTok{$}\NormalTok{weight}
\NormalTok{  )}
\NormalTok{\}}

\NormalTok{cv\_ref }\OtherTok{\textless{}{-}} \FunctionTok{run\_cv}\NormalTok{(soil\_df\_ref, }\AttributeTok{response =} \StringTok{"SOC\_0\_30"}\NormalTok{, }\AttributeTok{predictors =}\NormalTok{ cov\_names)}
\NormalTok{cv\_aug }\OtherTok{\textless{}{-}} \FunctionTok{run\_cv}\NormalTok{(soil\_df\_aug, }\AttributeTok{response =} \StringTok{"SOC\_0\_30"}\NormalTok{, }\AttributeTok{predictors =}\NormalTok{ cov\_names)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{compute\_metrics }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(cv) \{}
  \FunctionTok{data.frame}\NormalTok{(}
    \AttributeTok{wME =} \FunctionTok{round}\NormalTok{(}\FunctionTok{wME}\NormalTok{(cv}\SpecialCharTok{$}\NormalTok{obs,   cv}\SpecialCharTok{$}\NormalTok{pred, cv}\SpecialCharTok{$}\NormalTok{weight), }\DecValTok{3}\NormalTok{),}
    \AttributeTok{wRMSE =} \FunctionTok{round}\NormalTok{(}\FunctionTok{wRMSE}\NormalTok{(cv}\SpecialCharTok{$}\NormalTok{obs, cv}\SpecialCharTok{$}\NormalTok{pred, cv}\SpecialCharTok{$}\NormalTok{weight), }\DecValTok{3}\NormalTok{),}
    \AttributeTok{wNSE =} \FunctionTok{round}\NormalTok{(}\FunctionTok{wNSE}\NormalTok{(cv}\SpecialCharTok{$}\NormalTok{obs,  cv}\SpecialCharTok{$}\NormalTok{pred, cv}\SpecialCharTok{$}\NormalTok{weight), }\DecValTok{3}\NormalTok{)}
\NormalTok{  )}
\NormalTok{\}}

\NormalTok{metrics }\OtherTok{\textless{}{-}} \FunctionTok{rbind}\NormalTok{(}
  \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{Model =} \StringTok{"Reference (lab only)"}\NormalTok{,  }\FunctionTok{compute\_metrics}\NormalTok{(cv\_ref)),}
  \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{Model =} \StringTok{"Augmented (lab + DRS)"}\NormalTok{, }\FunctionTok{compute\_metrics}\NormalTok{(cv\_aug))}
\NormalTok{)}
...
{\centering \includegraphics[width=0.5\linewidth]{images/module4/fig_msd_workflow} 

}

\caption{Workflow for identifying the optimal calibration set size using the mean squared distance between spectral density estimates.}\label{fig:fig-msd-workflow}
\end{figure}
...
\NormalTok{pca\_out }\OtherTok{\textless{}{-}} \FunctionTok{prcomp}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{spc\_processed, }\AttributeTok{center =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{scale. =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{scores\_raw }\OtherTok{\textless{}{-}}\NormalTok{ pca\_out}\SpecialCharTok{$}\NormalTok{x[, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{]}

\CommentTok{\# Scale each PC by its standard deviation}
\NormalTok{scores\_sd }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(scores\_raw, }\DecValTok{2}\NormalTok{, sd)}
\NormalTok{scores  }\OtherTok{\textless{}{-}} \FunctionTok{sweep}\NormalTok{(scores\_raw, }\DecValTok{2}\NormalTok{, scores\_sd, }\StringTok{"/"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n\_grid   }\OtherTok{\textless{}{-}} \DecValTok{512}
\NormalTok{kde\_full }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{ncol}\NormalTok{(scores))}

\ControlFlowTok{for}\NormalTok{ (j }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(}\FunctionTok{ncol}\NormalTok{(scores))) \{}
\NormalTok{  kde\_full[[j]] }\OtherTok{\textless{}{-}} \FunctionTok{density}\NormalTok{(}
\NormalTok{    scores[, j],}
    \AttributeTok{bw =} \StringTok{"nrd0"}\NormalTok{,}
    \AttributeTok{n =}\NormalTok{ n\_grid,}
    \AttributeTok{from =} \FunctionTok{min}\NormalTok{(scores[, j]),}
    \AttributeTok{to =} \FunctionTok{max}\NormalTok{(scores[, j])}
\NormalTok{  )}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Candidate calibration set sizes to evaluate (number of k{-}means centres)}
\NormalTok{set\_sizes }\OtherTok{\textless{}{-}} \FunctionTok{seq}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{200}\NormalTok{, }\AttributeTok{by =} \DecValTok{10}\NormalTok{)}

\CommentTok{\# Number of repetitions per set size to account for k{-}means stochasticity}
\NormalTok{repetitions }\OtherTok{\textless{}{-}} \DecValTok{10}

\CommentTok{\# Matrix to store MSD values: rows = set sizes, columns = repetitions}
\NormalTok{msd\_matrix }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\ConstantTok{NA}\NormalTok{, }\AttributeTok{nrow =} \FunctionTok{length}\NormalTok{(set\_sizes), }\AttributeTok{ncol =}\NormalTok{ repetitions)}

\CommentTok{\# KDE of PC1 for each set size (first repetition only, used for plotting)}
\NormalTok{kde\_subsets\_pc1 }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(set\_sizes))}

\CommentTok{\# Average number of horizon samples after profile completion, per set size}
\NormalTok{n\_samples\_vec }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(}\FunctionTok{length}\NormalTok{(set\_sizes))}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(set\_sizes)) \{}
  \CommentTok{\# Store sample counts across repetitions to compute the average}
\NormalTok{  n\_samp\_rep }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(repetitions)}
  
  \ControlFlowTok{for}\NormalTok{ (r }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(repetitions)) \{}
    \CommentTok{\# Different seed per repetition and set size for independent realisations}
    \FunctionTok{set.seed}\NormalTok{(r }\SpecialCharTok{*}\NormalTok{ i)}
    \CommentTok{\# k{-}means sampling in PC score space}
\NormalTok{    kms\_i }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{naes}\NormalTok{(}
\NormalTok{      scores,}
      \AttributeTok{k =}\NormalTok{ set\_sizes[i],}
      \AttributeTok{iter.max =} \DecValTok{1000}
\NormalTok{    )}
    
    \CommentTok{\# Profile{-}complete the selection: include all horizons from the profiles}
    \CommentTok{\# of the selected samples to avoid splitting profiles across sets}
\NormalTok{    idx\_i }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%} \FunctionTok{unique}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID[kms\_i}\SpecialCharTok{$}\NormalTok{model]))}
\NormalTok{    n\_samp\_rep[r] }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(idx\_i)}
    
    \CommentTok{\# Compute MSD for each PC separately, then average across components}
    \CommentTok{\# The same bandwidth and grid as the full{-}population KDE are used}
    \CommentTok{\# so that differences in MSD reflect distributional differences only}
\NormalTok{    msd\_pcs }\OtherTok{\textless{}{-}} \FunctionTok{numeric}\NormalTok{(}\FunctionTok{ncol}\NormalTok{(scores))}
    \ControlFlowTok{for}\NormalTok{ (j }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(}\FunctionTok{ncol}\NormalTok{(scores))) \{}
\NormalTok{      kde\_j }\OtherTok{\textless{}{-}} \FunctionTok{density}\NormalTok{(}
\NormalTok{        scores[idx\_i, j],}
        \AttributeTok{bw =}\NormalTok{ kde\_full[[j]]}\SpecialCharTok{$}\NormalTok{bw,}
        \AttributeTok{n =}\NormalTok{ n\_grid,}
        \AttributeTok{from =} \FunctionTok{min}\NormalTok{(scores[, j]),}
        \AttributeTok{to =} \FunctionTok{max}\NormalTok{(scores[, j])}
\NormalTok{      )}
\NormalTok{      msd\_pcs[j] }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{((kde\_j}\SpecialCharTok{$}\NormalTok{y }\SpecialCharTok{{-}}\NormalTok{ kde\_full[[j]]}\SpecialCharTok{$}\NormalTok{y)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\NormalTok{    \}}
    \CommentTok{\# Average MSD across all 10 PCs for this repetition and set size}
\NormalTok{    msd\_matrix[i, r] }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(msd\_pcs)}
    
    \CommentTok{\# Store PC1 KDE for the first repetition only (used in the density plot)}
    \ControlFlowTok{if}\NormalTok{ (r }\SpecialCharTok{==} \DecValTok{1}\NormalTok{) \{}
\NormalTok{      kde\_subsets\_pc1[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{density}\NormalTok{(}
\NormalTok{        scores[idx\_i, }\DecValTok{1}\NormalTok{],}
        \AttributeTok{bw =}\NormalTok{ kde\_full[[}\DecValTok{1}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{bw,}
        \AttributeTok{n =}\NormalTok{ n\_grid,}
        \AttributeTok{from =} \FunctionTok{min}\NormalTok{(scores[, }\DecValTok{1}\NormalTok{]),}
        \AttributeTok{to =} \FunctionTok{max}\NormalTok{(scores[, }\DecValTok{1}\NormalTok{])}
\NormalTok{      )}
\NormalTok{    \}}
\NormalTok{  \}}
  \CommentTok{\# Average number of samples across repetitions for the dual x{-}axis labels}
\NormalTok{  n\_samples\_vec[i] }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(}\FunctionTok{mean}\NormalTok{(n\_samp\_rep))}
\NormalTok{\}}

\CommentTok{\# Summarise MSD across repetitions: mean and SD per set size}
\NormalTok{msd\_mean }\OtherTok{\textless{}{-}} \FunctionTok{rowMeans}\NormalTok{(msd\_matrix)}
\NormalTok{msd\_sd }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(msd\_matrix, }\DecValTok{1}\NormalTok{, sd)}
\end{Highlighting}
\end{Shaded}

\subsubsection{Step 4: Visualising distributional convergence}\label{step-4-visualising-distributional-convergence}

Figure (\citeproc{ref-ref}{\textbf{ref?}})(fig:msd-plot-density) illustrates the convergence of the calibration subset distribution toward the full population distribution, shown here for the first principal component as a representative example. The MSD curve in Figure (\citeproc{ref-ref}{\textbf{ref?}})(fig:msd-plot-curve) integrates this comparison across all 10 retained components, each contributing equally to the average since the scores are scaled to unit variance.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(}
\NormalTok{  kde\_full[[}\DecValTok{1}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{x, kde\_full[[}\DecValTok{1}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{y,}
  \AttributeTok{type =} \StringTok{"l"}\NormalTok{, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"black"}\NormalTok{,}
  \AttributeTok{xlab =} \StringTok{"PC 1 (scaled)"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Density"}
\NormalTok{)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(set\_sizes)) \{}
  \FunctionTok{lines}\NormalTok{(}
\NormalTok{    kde\_subsets\_pc1[[i]]}\SpecialCharTok{$}\NormalTok{x,}
\NormalTok{    kde\_subsets\_pc1[[i]]}\SpecialCharTok{$}\NormalTok{y,}
    \AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.20}\NormalTok{, }\FloatTok{0.45}\NormalTok{, }\FloatTok{0.70}\NormalTok{, }\FloatTok{0.25}\NormalTok{),}
    \AttributeTok{lwd =} \DecValTok{1}
\NormalTok{  )}
\NormalTok{\}}

\FunctionTok{lines}\NormalTok{(kde\_full[[}\DecValTok{1}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{x, kde\_full[[}\DecValTok{1}\NormalTok{]]}\SpecialCharTok{$}\NormalTok{y, }\AttributeTok{lwd =} \DecValTok{2}\NormalTok{, }\AttributeTok{col =} \StringTok{"black"}\NormalTok{)}

\FunctionTok{legend}\NormalTok{(}
  \StringTok{"topleft"}\NormalTok{,}
  \AttributeTok{legend =} \FunctionTok{c}\NormalTok{(}\StringTok{"Full set"}\NormalTok{, }\StringTok{"Calibration subsets"}\NormalTok{),}
  \AttributeTok{col =} \FunctionTok{c}\NormalTok{(}\StringTok{"black"}\NormalTok{, }\FunctionTok{rgb}\NormalTok{(}\FloatTok{0.20}\NormalTok{, }\FloatTok{0.45}\NormalTok{, }\FloatTok{0.70}\NormalTok{, }\FloatTok{0.6}\NormalTok{)),}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/msd-plot-density-1} 

}

\caption{Empirical distributions of the first principal component of the processed MIR spectra. The solid black line shows the full population; semi-transparent blue lines show calibration subsets of increasing size.}\label{fig:msd-plot-density}
\end{figure}
...
The MSD curve declines steeply up to approximately 50--60 profiles (\textasciitilde330--390 samples), after which the rate of decrease slows substantially. This suggests that around 50--60 profiles captures the main spectral variability of the population, with further additions producing diminishing returns.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Extended margins to accommodate both axes and rotated labels}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\DecValTok{7}\NormalTok{, }\DecValTok{5}\NormalTok{, }\DecValTok{6}\NormalTok{, }\DecValTok{2}\NormalTok{))}

\CommentTok{\# Base plot with sample counts on bottom x axis}
\FunctionTok{plot}\NormalTok{(}
  \AttributeTok{x =}\NormalTok{ set\_sizes,}
  \AttributeTok{y =}\NormalTok{ msd\_mean,}
  \AttributeTok{type =} \StringTok{"n"}\NormalTok{,}
  \AttributeTok{xlab =} \StringTok{""}\NormalTok{,}
  \AttributeTok{ylab =} \StringTok{"Mean squared distance between density estimates"}\NormalTok{,}
  \AttributeTok{xaxt =} \StringTok{"n"}\NormalTok{,}
  \AttributeTok{ylim =} \FunctionTok{range}\NormalTok{(}\FunctionTok{c}\NormalTok{(msd\_mean }\SpecialCharTok{{-}}\NormalTok{ msd\_sd, msd\_mean }\SpecialCharTok{+}\NormalTok{ msd\_sd))}
\NormalTok{)}

\CommentTok{\# uncertainty band (±1 SD)}
\FunctionTok{polygon}\NormalTok{(}
  \AttributeTok{x =} \FunctionTok{c}\NormalTok{(set\_sizes, }\FunctionTok{rev}\NormalTok{(set\_sizes)),}
  \AttributeTok{y =} \FunctionTok{c}\NormalTok{(msd\_mean }\SpecialCharTok{{-}}\NormalTok{ msd\_sd, }\FunctionTok{rev}\NormalTok{(msd\_mean }\SpecialCharTok{+}\NormalTok{ msd\_sd)),}
  \AttributeTok{col =} \FunctionTok{adjustcolor}\NormalTok{(}\StringTok{"\#4E84A8"}\NormalTok{, }\AttributeTok{alpha.f =} \FloatTok{0.25}\NormalTok{),}
  \AttributeTok{border =} \ConstantTok{NA}
\NormalTok{)}

\CommentTok{\# Mean MSD line}
\FunctionTok{lines}\NormalTok{(set\_sizes, msd\_mean, }\AttributeTok{col =} \StringTok{"\#4E84A8"}\NormalTok{, }\AttributeTok{lwd =} \FloatTok{1.5}\NormalTok{)}

\CommentTok{\# Bottom axis: approximate number of samples (rotated labels)}
\FunctionTok{axis}\NormalTok{(}
  \AttributeTok{side =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{at =}\NormalTok{ set\_sizes,}
  \AttributeTok{labels =}\NormalTok{ n\_samples\_vec,}
  \AttributeTok{las =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{cex.axis =} \FloatTok{0.8}
\NormalTok{)}
\FunctionTok{mtext}\NormalTok{(}
  \StringTok{"Number of samples in calibration set"}\NormalTok{,}
  \AttributeTok{side =} \DecValTok{1}\NormalTok{, }\AttributeTok{line =} \FloatTok{5.5}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.9}
\NormalTok{)}

\CommentTok{\# Top axis: number of profiles}
\FunctionTok{axis}\NormalTok{(}
  \AttributeTok{side =} \DecValTok{3}\NormalTok{,}
  \AttributeTok{at =}\NormalTok{ set\_sizes,}
  \AttributeTok{labels =}\NormalTok{ set\_sizes,}
  \AttributeTok{las =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{cex.axis =} \FloatTok{0.8}
\NormalTok{)}
\FunctionTok{mtext}\NormalTok{(}
  \StringTok{"Number of profiles in calibration set"}\NormalTok{,}
  \AttributeTok{side =} \DecValTok{3}\NormalTok{, }\AttributeTok{line =} \FloatTok{4.5}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.9}
\NormalTok{)}

\FunctionTok{grid}\NormalTok{(}\AttributeTok{lty =} \DecValTok{1}\NormalTok{, }\AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/msd-plot-curve-1} 

}

\caption{Mean squared distance between the spectral density estimates of calibration subsets and the full population, as a function of calibration set size. The ribbon shows one standard deviation across repetitions. The bottom axis shows the approximate number of horizon samples; the top axis shows the corresponding number of soil profiles.}\label{fig:msd-plot-curve}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{n\_profiles }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\DecValTok{20}\NormalTok{, }\DecValTok{50}\NormalTok{, }\DecValTok{85}\NormalTok{, }\DecValTok{140}\NormalTok{, }\DecValTok{200}\NormalTok{, }\DecValTok{300}\NormalTok{)}

\CommentTok{\# For each calibration set size, select profiles using k{-}means sampling}
\CommentTok{\# and store the indices of the calibration and prediction samples}
\NormalTok{cal\_indices }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\FunctionTok{names}\NormalTok{(cal\_indices) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  \FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{Selecting"}\NormalTok{, n\_profiles[i], }\StringTok{"profiles}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
  \FunctionTok{set.seed}\NormalTok{(}\DecValTok{201909}\NormalTok{)}
\NormalTok{  kms\_i }\OtherTok{\textless{}{-}}\NormalTok{ prospectr}\SpecialCharTok{::}\FunctionTok{naes}\NormalTok{(}
\NormalTok{    dat}\SpecialCharTok{$}\NormalTok{spc\_processed,}
    \AttributeTok{k =}\NormalTok{ n\_profiles[i],}
    \AttributeTok{pc =} \DecValTok{10}\NormalTok{,}
    \AttributeTok{iter.max =} \DecValTok{1000}
\NormalTok{  )}
  \CommentTok{\# Profile{-}complete the selection}
\NormalTok{  cal\_idx }\OtherTok{\textless{}{-}} \FunctionTok{which}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%} \FunctionTok{unique}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID[kms\_i}\SpecialCharTok{$}\NormalTok{model]))}
\NormalTok{  cal\_indices[[i]] }\OtherTok{\textless{}{-}}\NormalTok{ cal\_idx}
  \FunctionTok{cat}\NormalTok{(}\StringTok{" Calibration samples:"}\NormalTok{, }\FunctionTok{length}\NormalTok{(cal\_idx), }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Storage for DRS predictions across calibration set sizes}
\NormalTok{drs\_preds }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\FunctionTok{names}\NormalTok{(drs\_preds) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  \FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{Fitting DRS model for"}\NormalTok{, n\_profiles[i], }\StringTok{"profiles}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{  cal\_idx }\OtherTok{\textless{}{-}}\NormalTok{ cal\_indices[[i]]}
\NormalTok{  pred\_idx }\OtherTok{\textless{}{-}} \FunctionTok{setdiff}\NormalTok{(}\FunctionTok{seq\_len}\NormalTok{(}\FunctionTok{nrow}\NormalTok{(dat)), cal\_idx)}
  
  \CommentTok{\# Fit quantile regression forest on calibration samples}
\NormalTok{  drs\_model\_i }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
    \AttributeTok{x =}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{spc\_processed[cal\_idx, ],}
    \AttributeTok{y =}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{SOC[cal\_idx],}
    \AttributeTok{quantreg =} \ConstantTok{TRUE}\NormalTok{,}
    \AttributeTok{num.trees =} \DecValTok{3500}\NormalTok{,}
    \AttributeTok{sample.fraction =} \DecValTok{1}\NormalTok{,}
    \AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{,}
...
\NormalTok{  )}
  ...
  \CommentTok{\# Predict SOC for non{-}calibration samples}
\NormalTok{  preds\_i }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(}
\NormalTok{    drs\_model\_i,}
    \AttributeTok{data =}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{spc\_processed[pred\_idx, ],}
    \AttributeTok{type =} \StringTok{"quantiles"}\NormalTok{,}
    \AttributeTok{what =} \ControlFlowTok{function}\NormalTok{(x) }\FunctionTok{sample}\NormalTok{(x, }\DecValTok{100}\NormalTok{, }\AttributeTok{replace =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{  )}\SpecialCharTok{$}\NormalTok{predictions}
  
  \CommentTok{\# Store mean prediction and prediction standard deviation}
\NormalTok{  drs\_preds[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
    \AttributeTok{row\_idx =}\NormalTok{ pred\_idx,}
    \AttributeTok{SOC\_predRF =} \FunctionTok{rowMeans}\NormalTok{(preds\_i),}
    \AttributeTok{SOC\_sdRF =} \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{rowVars}\NormalTok{(preds\_i))}
\NormalTok{  )}
  \FunctionTok{cat}\NormalTok{(}\StringTok{" "}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(drs\_preds[[i]]), }\StringTok{"samples predicted}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Storage for augmented datasets}
\NormalTok{aug\_datasets }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\FunctionTok{names}\NormalTok{(aug\_datasets) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}
\NormalTok{my\_col\_names }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}
  \StringTok{"smp\_id"}\NormalTok{, }
  \StringTok{"ProfID"}\NormalTok{, }
  \StringTok{"Long\_Site.x"}\NormalTok{, }
  \StringTok{"Lat\_Site.x"}\NormalTok{,}
  \StringTok{"Top\_depth\_cm.x"}\NormalTok{, }
  \StringTok{"Bottom\_depth\_cm.x"}
\NormalTok{)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
\NormalTok{  cal\_idx }\OtherTok{\textless{}{-}}\NormalTok{ cal\_indices[[i]]}
\NormalTok{  pred\_idx }\OtherTok{\textless{}{-}}\NormalTok{ drs\_preds[[i]]}\SpecialCharTok{$}\NormalTok{row\_idx}
  
  \CommentTok{\# Initialise augmented dataset with metadata columns only}
\NormalTok{  aug\_i }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(dat)[, my\_col\_names]}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC }\OtherTok{\textless{}{-}} \ConstantTok{NA}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd }\OtherTok{\textless{}{-}} \ConstantTok{NA}
  
  \CommentTok{\# Laboratory observations for calibration profiles}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC[cal\_idx] }\OtherTok{\textless{}{-}}\NormalTok{ dat}\SpecialCharTok{$}\NormalTok{SOC[cal\_idx]}
  \CommentTok{\# Assuming a constant standard deviation for lab measurements}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd[cal\_idx] }\OtherTok{\textless{}{-}} \FloatTok{0.15}   
  
  \CommentTok{\# DRS{-}based predictions for non{-}calibration samples}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC[pred\_idx] }\OtherTok{\textless{}{-}}\NormalTok{ drs\_preds[[i]]}\SpecialCharTok{$}\NormalTok{SOC\_predRF}
\NormalTok{  aug\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd[pred\_idx] }\OtherTok{\textless{}{-}}\NormalTok{ drs\_preds[[i]]}\SpecialCharTok{$}\NormalTok{SOC\_sdRF}
  
\NormalTok{  aug\_datasets[[i]] }\OtherTok{\textless{}{-}}\NormalTok{ aug\_i}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
We can use \texttt{soil\_df\_ref} directly for this purpose, since it already contains depth-harmonized SOC values derived exclusively from laboratory measurements (the same dataset used to fit the baseline DSM model). We simply sample a subset of its profiles as the held-out validation set.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Add ProfID to soil\_df\_ref to enable profile{-}level matching}
\NormalTok{soil\_df\_ref}\SpecialCharTok{$}\NormalTok{ProfID }\OtherTok{\textless{}{-}}\NormalTok{ soc\_ref\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_0\_30[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(soc\_ref\_0\_30\_xy}\SpecialCharTok{$}\NormalTok{SOC\_0\_30)]}

\CommentTok{\# Sample 50 profiles for held{-}out spatial validation}
\CommentTok{\# These profiles are never used in any calibration or augmented dataset}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{202409}\NormalTok{)}
\NormalTok{val\_profiles }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(}\FunctionTok{unique}\NormalTok{(soil\_df\_ref}\SpecialCharTok{$}\NormalTok{ProfID), }\AttributeTok{size =} \DecValTok{50}\NormalTok{)}
\NormalTok{val\_data }\OtherTok{\textless{}{-}}\NormalTok{ soil\_df\_ref[soil\_df\_ref}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%}\NormalTok{ val\_profiles, ]}

\FunctionTok{cat}\NormalTok{(}\StringTok{"Validation profiles:"}\NormalTok{, }\FunctionTok{length}\NormalTok{(val\_profiles), }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\FunctionTok{cat}\NormalTok{(}
  \StringTok{"Remaining profiles available for DSM modelling:"}\NormalTok{,}
  \FunctionTok{nrow}\NormalTok{(soil\_df\_ref) }\SpecialCharTok{{-}} \FunctionTok{nrow}\NormalTok{(val\_data), }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Helper function: depth{-}weighted mean SOC for 0{-}30 cm}
\CommentTok{\# Returns NA for profiles not fully covering the interval}
\NormalTok{depth\_weighted\_mean\_0\_30 }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(p) \{}
\NormalTok{  h }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(p)}
\NormalTok{  w }\OtherTok{\textless{}{-}} \FunctionTok{pmax}\NormalTok{(}\DecValTok{0}\NormalTok{, }\FunctionTok{pmin}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Bottom\_depth\_cm.x, }\DecValTok{30}\NormalTok{) }\SpecialCharTok{{-}} \FunctionTok{pmax}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{Top\_depth\_cm.x, }\DecValTok{0}\NormalTok{))}
\NormalTok{  keep }\OtherTok{\textless{}{-}}\NormalTok{ w }\SpecialCharTok{\textgreater{}} \DecValTok{0} \SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(h}\SpecialCharTok{$}\NormalTok{SOC)}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{sum}\NormalTok{(w[keep]) }\SpecialCharTok{\textless{}} \DecValTok{30}\NormalTok{) }
...
\CommentTok{\# Storage for depth{-}harmonised augmented datasets}
\NormalTok{aug\_0\_30 }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\FunctionTok{names}\NormalTok{(aug\_0\_30) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
\NormalTok{  cal\_idx }\OtherTok{\textless{}{-}}\NormalTok{ cal\_indices[[i]]}
\NormalTok{  aug\_i }\OtherTok{\textless{}{-}}\NormalTok{ aug\_datasets[[i]]}
  \FunctionTok{depths}\NormalTok{(aug\_i) }\OtherTok{\textless{}{-}}\NormalTok{ ProfID }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Top\_depth\_cm.x }\SpecialCharTok{+}\NormalTok{ Bottom\_depth\_cm.x}
  
  \CommentTok{\# Depth{-}weighted mean SOC over 0{-}30 cm per profile}
\NormalTok{  soc\_mean\_i }\OtherTok{\textless{}{-}} \FunctionTok{profileApply}\NormalTok{(aug\_i, depth\_weighted\_mean\_0\_30)}
  
  \CommentTok{\# Depth{-}weighted propagation of SOC uncertainty over 0{-}30 cm}
\NormalTok{  soc\_sd\_i }\OtherTok{\textless{}{-}} \FunctionTok{horizons}\NormalTok{(aug\_i) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{as.data.frame}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{group\_by}\NormalTok{(ProfID) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{summarise}\NormalTok{(}
      \AttributeTok{SOC\_sd\_0\_30 =} \FunctionTok{weighted\_sd\_0\_30}\NormalTok{(Top\_depth\_cm.x, Bottom\_depth\_cm.x, SOC\_sd),}
...
\NormalTok{    )}
  ...
  \CommentTok{\# One set of coordinates per profile for spatial modelling  }
\NormalTok{  coords\_i }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(aug\_i)[, }\FunctionTok{c}\NormalTok{(}\StringTok{"ProfID"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{, }\StringTok{"Long\_Site.x"}\NormalTok{)]}
\NormalTok{  coords\_i }\OtherTok{\textless{}{-}}\NormalTok{ coords\_i[}\SpecialCharTok{!}\FunctionTok{duplicated}\NormalTok{(coords\_i}\SpecialCharTok{$}\NormalTok{ProfID), ]}
  
  \CommentTok{\# Combine mean SOC, uncertainty, and coordinates into a profile{-}level table}
\NormalTok{  soc\_0\_30\_i }\OtherTok{\textless{}{-}}\NormalTok{ soc\_sd\_i }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{mutate}\NormalTok{(}\AttributeTok{SOC\_0\_30 =}\NormalTok{ soc\_mean\_i) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{select}\NormalTok{(ProfID, SOC\_0\_30, SOC\_sd\_0\_30) }\SpecialCharTok{|\textgreater{}}
    \FunctionTok{merge}\NormalTok{(coords\_i, }\AttributeTok{by =} \StringTok{"ProfID"}\NormalTok{, }\AttributeTok{all.x =} \ConstantTok{TRUE}\NormalTok{)}
  
  \CommentTok{\# Reassign constant laboratory uncertainty at profile level to avoid}
  \CommentTok{\# the variance{-}reduction artefact from depth{-}weighted propagation}
\NormalTok{  cal\_profiles }\OtherTok{\textless{}{-}} \FunctionTok{unique}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{ProfID[cal\_idx])}
\NormalTok{  soc\_0\_30\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30[soc\_0\_30\_i}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%}\NormalTok{ cal\_profiles] }\OtherTok{\textless{}{-}} \FloatTok{0.15}
  
  \CommentTok{\# Remove held{-}out validation profiles from the augmented dataset}
\NormalTok{  aug\_0\_30[[i]] }\OtherTok{\textless{}{-}}\NormalTok{ soc\_0\_30\_i[}\SpecialCharTok{!}\NormalTok{soc\_0\_30\_i}\SpecialCharTok{$}\NormalTok{ProfID }\SpecialCharTok{\%in\%}\NormalTok{ val\_profiles, ]}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Storage for DSM models and predicted SOC maps}
\NormalTok{dsm\_models }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\NormalTok{soc\_maps }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}
\FunctionTok{names}\NormalTok{(dsm\_models) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}
\FunctionTok{names}\NormalTok{(soc\_maps) }\OtherTok{\textless{}{-}} \FunctionTok{as.character}\NormalTok{(n\_profiles)}

\NormalTok{my\_formula }\OtherTok{\textless{}{-}} \FunctionTok{as.formula}\NormalTok{(}
  \FunctionTok{paste}\NormalTok{(}\StringTok{"SOC\_0\_30 \textasciitilde{}"}\NormalTok{, }\FunctionTok{paste}\NormalTok{(cov\_names, }\AttributeTok{collapse =} \StringTok{" + "}\NormalTok{))}
\NormalTok{)}
\NormalTok{nms }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"x"}\NormalTok{, }\StringTok{"y"}\NormalTok{, }\StringTok{"SOC\_0\_30"}\NormalTok{, }\StringTok{"SOC\_sd\_0\_30"}\NormalTok{, cov\_names)}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  \ControlFlowTok{if}\NormalTok{ (i }\SpecialCharTok{==} \DecValTok{1}\NormalTok{) \{}
      \FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{This may take a while... how about a break? \textbackslash{}u2615\textbackslash{}u2615}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{  \} }
  \FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{Fitting DSM model for"}\NormalTok{, n\_profiles[i], }\StringTok{"profiles}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
  \CommentTok{\# Convert to spatial points and extract covariates}
\NormalTok{  pts\_i }\OtherTok{\textless{}{-}} \FunctionTok{vect}\NormalTok{(}
\NormalTok{    aug\_0\_30[[i]],}
    \AttributeTok{geom =} \FunctionTok{c}\NormalTok{(}\StringTok{"Long\_Site.x"}\NormalTok{, }\StringTok{"Lat\_Site.x"}\NormalTok{),}
    \AttributeTok{crs =} \StringTok{"EPSG:4326"}
\NormalTok{  )}
\NormalTok{  pts\_i }\OtherTok{\textless{}{-}} \FunctionTok{project}\NormalTok{(pts\_i, covs)}
\NormalTok{  pts\_i }\OtherTok{\textless{}{-}} \FunctionTok{cbind}\NormalTok{(pts\_i, terra}\SpecialCharTok{::}\FunctionTok{extract}\NormalTok{(covs, pts\_i)[, }\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{])}
  
  \CommentTok{\# Prepare modelling table and compute inverse{-}variance weights}
\NormalTok{  df\_i }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(pts\_i, }\AttributeTok{geom =} \StringTok{"XY"}\NormalTok{)[ , nms]}
\NormalTok{  df\_i }\OtherTok{\textless{}{-}}\NormalTok{ df\_i[}\SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(df\_i}\SpecialCharTok{$}\NormalTok{SOC\_0\_30) }\SpecialCharTok{\&} \SpecialCharTok{!}\FunctionTok{is.na}\NormalTok{(df\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30), ]}
\NormalTok{  df\_i}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{/}\NormalTok{ df\_i}\SpecialCharTok{$}\NormalTok{SOC\_sd\_0\_30}\SpecialCharTok{\^{}}\DecValTok{2}
\NormalTok{  df\_i}\SpecialCharTok{$}\NormalTok{weight }\OtherTok{\textless{}{-}}\NormalTok{ df\_i}\SpecialCharTok{$}\NormalTok{weight }\SpecialCharTok{/} \FunctionTok{max}\NormalTok{(df\_i}\SpecialCharTok{$}\NormalTok{weight, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
  
  \CommentTok{\# Fit weighted random forest DSM model}
\NormalTok{  dsm\_models[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{ranger}\NormalTok{(}
    \AttributeTok{formula =}\NormalTok{ my\_formula,}
    \AttributeTok{data =}\NormalTok{ df\_i,}
    \AttributeTok{case.weights =}\NormalTok{ df\_i}\SpecialCharTok{$}\NormalTok{weight,}
    \AttributeTok{replace =} \ConstantTok{FALSE}\NormalTok{,}
    \AttributeTok{sample.fraction =} \FloatTok{0.632}\NormalTok{,}
    \AttributeTok{num.trees =} \DecValTok{500}\NormalTok{,}
    \AttributeTok{seed =} \DecValTok{201909}
\NormalTok{  )}
  \CommentTok{\# Predict SOC across the covariate raster stack}
\NormalTok{  soc\_maps[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(covs, dsm\_models[[i]], }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{par}\NormalTok{(}
  \AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\FunctionTok{length}\NormalTok{(n\_profiles), }\DecValTok{2}\NormalTok{),}
  \AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\FloatTok{1.5}\NormalTok{, }\DecValTok{3}\NormalTok{),}
  \AttributeTok{oma =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{)}
\NormalTok{)}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  
\NormalTok{  n\_cal\_samples }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(cal\_indices[[i]])}
  
  \FunctionTok{plot}\NormalTok{(}
\NormalTok{    soc\_maps[[i]],}
    \AttributeTok{main =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"DRS cal.: "}\NormalTok{, n\_profiles[i], }\StringTok{" profiles ("}\NormalTok{, n\_cal\_samples, }\StringTok{" samples)"}\NormalTok{),}
    \AttributeTok{range =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{)}
\NormalTok{  )}
  \FunctionTok{plot}\NormalTok{(}
\NormalTok{    soc\_maps[[i]] }\SpecialCharTok{{-}}\NormalTok{ soc\_map\_ref,}
    \AttributeTok{main =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"Difference: "}\NormalTok{, n\_profiles[i], }\StringTok{" profiles ("}\NormalTok{, n\_cal\_samples, }\StringTok{" samples)"}\NormalTok{),}
    \AttributeTok{col =} \FunctionTok{rev}\NormalTok{(}\FunctionTok{hcl.colors}\NormalTok{(}\DecValTok{100}\NormalTok{, }\StringTok{"RdBu"}\NormalTok{)),}
    \AttributeTok{range =} \FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\FloatTok{1.5}\NormalTok{, }\FloatTok{1.5}\NormalTok{)}
\NormalTok{  )}
\NormalTok{\}}

\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{), }\AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{2}\NormalTok{), }\AttributeTok{oma =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{0}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{map\_theme }\OtherTok{\textless{}{-}} \FunctionTok{theme\_void}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}
    ...
    \AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{size =} \DecValTok{9}\NormalTok{),}
...
\CommentTok{\# Build one row per calibration set size: SOC map (left) + difference map (right)}
\NormalTok{plot\_rows }\OtherTok{\textless{}{-}} \FunctionTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{, }\FunctionTok{length}\NormalTok{(n\_profiles))}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  
\NormalTok{  n\_cal\_samples }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(cal\_indices[[i]])}
  
  \CommentTok{\# Convert rasters to data frames}
\NormalTok{  df\_aug\_i }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(soc\_maps[[i]], }\AttributeTok{xy =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{  df\_diff\_i }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(soc\_maps[[i]] }\SpecialCharTok{{-}}\NormalTok{ soc\_map\_ref, }\AttributeTok{xy =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
  \FunctionTok{names}\NormalTok{(df\_aug\_i)[}\DecValTok{3}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{"SOC"}
  \FunctionTok{names}\NormalTok{(df\_diff\_i)[}\DecValTok{3}\NormalTok{] }\OtherTok{\textless{}{-}} \StringTok{"SOC"}
  
  \CommentTok{\# Left panel: augmented SOC map}
\NormalTok{  p\_aug\_i }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df\_aug\_i, }\FunctionTok{aes}\NormalTok{(x, y, }\AttributeTok{fill =}\NormalTok{ SOC)) }\SpecialCharTok{+}
    \FunctionTok{geom\_raster}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{scale\_fill\_viridis\_c}\NormalTok{(}\AttributeTok{limits =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{4}\NormalTok{), }\AttributeTok{name =} \StringTok{"SOC (\%)"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{coord\_equal}\NormalTok{() }\SpecialCharTok{+}\NormalTok{ map\_theme }\SpecialCharTok{+}
    \FunctionTok{ggtitle}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"DRS cal.: "}\NormalTok{, n\_profiles[i], }\StringTok{" profiles ("}\NormalTok{, n\_cal\_samples, }\StringTok{" samples)"}\NormalTok{))}
  
  \CommentTok{\# Right panel: difference map (augmented minus baseline)}
\NormalTok{  p\_diff\_i }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(df\_diff\_i, }\FunctionTok{aes}\NormalTok{(x, y, }\AttributeTok{fill =}\NormalTok{ SOC)) }\SpecialCharTok{+}
    \FunctionTok{geom\_raster}\NormalTok{() }\SpecialCharTok{+}
    \FunctionTok{scale\_fill\_gradient2}\NormalTok{(}
      ...
      \AttributeTok{name =} \StringTok{"Difference (\%)"}
\NormalTok{    ) }\SpecialCharTok{+}
    \FunctionTok{coord\_equal}\NormalTok{() }\SpecialCharTok{+}\NormalTok{ map\_theme }\SpecialCharTok{+}
    \FunctionTok{ggtitle}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"Difference: "}\NormalTok{, n\_profiles[i], }\StringTok{" profiles ("}\NormalTok{, n\_cal\_samples, }\StringTok{" samples)"}\NormalTok{))}
  
  \CommentTok{\# Combine into one row}
\NormalTok{  plot\_rows[[i]] }\OtherTok{\textless{}{-}} \FunctionTok{ggarrange}\NormalTok{(}
\NormalTok{    p\_aug\_i, p\_diff\_i,}
    \AttributeTok{ncol =} \DecValTok{2}\NormalTok{, }\AttributeTok{nrow =} \DecValTok{1}
\NormalTok{  )}
\NormalTok{\}}

\CommentTok{\# Stack all rows into a single figure}
\FunctionTok{ggarrange}\NormalTok{(}
  \AttributeTok{plotlist =}\NormalTok{ plot\_rows,}
  \AttributeTok{ncol =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{nrow =} \FunctionTok{length}\NormalTok{(n\_profiles)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-drs4dsm-visualise-1} 

}

\caption{Comparison of SOC maps derived from laboratory-only observations and DRS-augmented datasets with varying calibration set sizes. The difference maps highlight areas where the inclusion of DRS-derived predictions had the largest influence on the spatial patterns of predicted SOC.}\label{fig:fig-drs4dsm-visualise}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Evaluate each DSM model on the held{-}out validation profiles}
\NormalTok{dsm\_metrics }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{()}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_along}\NormalTok{(n\_profiles)) \{}
  \CommentTok{\# Predict SOC at validation locations using the fitted DSM model}
\NormalTok{  preds\_val }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(dsm\_models[[i]], }\AttributeTok{data =}\NormalTok{ val\_data)}\SpecialCharTok{$}\NormalTok{predictions}
  
\NormalTok{  obs }\OtherTok{\textless{}{-}}\NormalTok{ val\_data}\SpecialCharTok{$}\NormalTok{SOC\_0\_30}
\NormalTok{  resid }\OtherTok{\textless{}{-}}\NormalTok{ preds\_val }\SpecialCharTok{{-}}\NormalTok{ obs}
  
\NormalTok{  me }\OtherTok{\textless{}{-}} \FunctionTok{mean}\NormalTok{(resid, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{  rmse }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(}\FunctionTok{mean}\NormalTok{(resid}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{))}
\NormalTok{  nse }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{sum}\NormalTok{(resid}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{/}
    \FunctionTok{sum}\NormalTok{((obs }\SpecialCharTok{{-}} \FunctionTok{mean}\NormalTok{(obs, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{))}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{)}
  
\NormalTok{  dsm\_metrics }\OtherTok{\textless{}{-}} \FunctionTok{rbind}\NormalTok{(}
\NormalTok{    dsm\_metrics,}
    \FunctionTok{data.frame}\NormalTok{(}
      \AttributeTok{n\_profiles =}\NormalTok{ n\_profiles[i],}
      \AttributeTok{n\_samples =} \FunctionTok{length}\NormalTok{(cal\_indices[[i]]),}
      \AttributeTok{ME =} \FunctionTok{round}\NormalTok{(me, }\DecValTok{3}\NormalTok{),}
      \AttributeTok{RMSE =} \FunctionTok{round}\NormalTok{(rmse, }\DecValTok{3}\NormalTok{),}
      \AttributeTok{NSE =} \FunctionTok{round}\NormalTok{(nse, }\DecValTok{3}\NormalTok{)}
\NormalTok{    )}
\NormalTok{  )}
\NormalTok{\}}
...
\FunctionTok{plot}\NormalTok{(}
 \AttributeTok{x =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{n\_samples,}
 \AttributeTok{y =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{RMSE,}
 \AttributeTok{type =} \StringTok{"b"}\NormalTok{,}
 \AttributeTok{pch =} \DecValTok{16}\NormalTok{,}
 \AttributeTok{col =} \StringTok{"\#4E84A8"}\NormalTok{,}
 \AttributeTok{lwd =} \FloatTok{1.5}\NormalTok{,}
 \AttributeTok{xlab =} \StringTok{""}\NormalTok{,}
 \AttributeTok{ylab =} \StringTok{"RMSE (\% SOC)"}\NormalTok{,}
 \AttributeTok{xaxt =} \StringTok{"n"}\NormalTok{,}
 \AttributeTok{ylim =} \FunctionTok{range}\NormalTok{(dsm\_metrics}\SpecialCharTok{$}\NormalTok{RMSE) }\SpecialCharTok{*} \FunctionTok{c}\NormalTok{(}\FloatTok{0.9}\NormalTok{, }\FloatTok{1.1}\NormalTok{)}
\NormalTok{)}

\FunctionTok{grid}\NormalTok{(}\AttributeTok{lty =} \DecValTok{1}\NormalTok{, }\AttributeTok{col =} \FunctionTok{rgb}\NormalTok{(}\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.5}\NormalTok{))}

\CommentTok{\# Bottom axis: number of calibration samples}
\FunctionTok{axis}\NormalTok{(}
 \AttributeTok{side =} \DecValTok{1}\NormalTok{,}
 \AttributeTok{at =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{n\_samples,}
 \AttributeTok{labels =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{n\_samples,}
 \AttributeTok{las =} \DecValTok{2}\NormalTok{,}
 \AttributeTok{cex.axis =} \FloatTok{0.8}
\NormalTok{)}
\FunctionTok{mtext}\NormalTok{(}\StringTok{"Number of samples in calibration set"}\NormalTok{, }\AttributeTok{side =} \DecValTok{1}\NormalTok{, }\AttributeTok{line =} \FloatTok{5.5}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.9}\NormalTok{)}

\CommentTok{\# Top axis: number of calibration profiles}
\FunctionTok{axis}\NormalTok{(}
 \AttributeTok{side =} \DecValTok{3}\NormalTok{,}
 \AttributeTok{at =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{n\_samples,}
 \AttributeTok{labels =}\NormalTok{ dsm\_metrics}\SpecialCharTok{$}\NormalTok{n\_profiles,}
 \AttributeTok{las =} \DecValTok{2}\NormalTok{,}
 \AttributeTok{cex.axis =} \FloatTok{0.8}
\NormalTok{)}
\FunctionTok{mtext}\NormalTok{(}\StringTok{"Number of profiles in calibration set"}\NormalTok{, }\AttributeTok{side =} \DecValTok{3}\NormalTok{, }\AttributeTok{line =} \FloatTok{4.5}\NormalTok{, }\AttributeTok{cex =} \FloatTok{0.9}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}

{\centering \includegraphics{SoilFER-Integrated-soil-information-for-decision-making_files/figure-latex/fig-drs4dsm-plot-metrics-1} 

}

\caption{DSM performance metrics (RMSE) on the held-out validation profiles as a function of the number of samples used to calibrate the DRS model. The bottom x-axis shows the number of calibration samples; the top x-axis shows the corresponding number of calibration profiles.}\label{fig:fig-drs4dsm-plot-metrics}
\end{figure}
...
\item
  Map the principal components of the MIR spectra (difficulty: medium). Rather than predicting SOC directly, the scores of the first few principal components of the spectra can be mapped spatially using the same DSM workflow. Each PC score map captures a distinct axis of spectral variation that reflects underlying soil compositional gradients (\citeproc{ref-viscarra2011soil}{Viscarra Rossel \emph{et al.}, 2011}). These maps can also be used as additional environmental covariates alongside the standard terrain and climate layers when predicting SOC.
\item
...
\section{How to add your draft}\label{how-to-add-your-draft}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Add your \texttt{.Rmd} file to this folder
\item
  It will \textbf{not} be compiled or published to the site automatically
\item
  When ready, move it to the root folder
\end{enumerate}

\section{Files in progress}\label{files-in-progress}
...
\bibitem[\citeproctext]{ref-chen2023integrating}
\textbf{Chen, S., Saby, N.P.A., Martin, M.P., Barthes, B.G., Gomez, C., Shi, Z. \& Arrouays, D.} 2023. Integrating additional spectroscopically inferred soil data improves the accuracy of digital soil mapping. \emph{Geoderma}, 433: 116467.
...
\bibitem[\citeproctext]{ref-dangal2019accurate}
\textbf{Dangal, S.R., Sanderman, J., Wills, S. \& Ramirez-Lopez, L.} 2019. Accurate and precise prediction of soil properties from a large mid-infrared spectral library. \emph{Soil Systems}, 3(1): 11.
...
\bibitem[\citeproctext]{ref-hengl2018random}
\textbf{Hengl, T., Nussbaum, M., Wright, M.N., Heuvelink, G.B. \& Gräler, B.} 2018. Random forest as a generic framework for predictive modeling of spatial and spatio-temporal variables. \emph{PeerJ}, 6: e5518.
...
\bibitem[\citeproctext]{ref-naes1987design}
\textbf{Næs, T.} 1987. The design of calibration in near infra-red reflectance analysis by clustering. \emph{Journal of chemometrics}, 1(2): 121--134.
...
\bibitem[\citeproctext]{ref-peng2025spectroscopic}
\textbf{Peng, Y., Ben-Dor, E., Biswas, A., Chabrillat, S., Demattê, J.A., Ge, Y., Gholizadeh, A., Gomez, C., Guerrero, C., Herrick, J. \& others}. 2025. Spectroscopic solutions for generating new global soil information. \emph{The Innovation}, 6(5).
...
\bibitem[\citeproctext]{ref-RamirezLopez2026Liblex}
\textbf{Ramirez-Lopez, L., Metz, M., Lesnoff, M., Orellano, C., Perez-Fernandez, E., Plans, M., Breure, T., Behrens, T., Viscarra Rossel, R. \& Peng, Y.} 2026. Rethinking local spectral modelling: From per-query refitting to model libraries. \emph{Analytica Chimica Acta}.

\bibitem[\citeproctext]{ref-ramirez2014sampling}
\textbf{Ramirez-Lopez, L., Schmidt, K., Behrens, T., Van Wesemael, B., Demattê, J.A. \& Scholten, T.} 2014. Sampling optimal calibration sets in soil infrared spectroscopy. \emph{Geoderma}, 226: 140--150.

\bibitem[\citeproctext]{ref-ramirez2019robust}
\textbf{Ramirez-Lopez, L., Wadoux, A.-C., Franceschini, M.H., Terra, F., Marques, K.P.P., Sayão, V.M. \& Demattê, J.A.M.} 2019. Robust soil mapping at the farm scale with vis--NIR spectroscopy. \emph{European Journal of Soil Science}, 70(2): 378--393.
...
\bibitem[\citeproctext]{ref-safanelli2025open}
\textbf{Safanelli, J.L., Hengl, T., Parente, L.L., Minarik, R., Bloom, D.E., Todd-Brown, K., Gholizadeh, A., Mendes, W. de S. \& Sanderman, J.} 2025. Open soil spectral library (OSSL): Building reproducible soil calibration models through open development and community engagement. \emph{PloS one}, 20(1): e0296545.
...
\bibitem[\citeproctext]{ref-stevens2013prediction}
\textbf{Stevens, A., Nocita, M., Tóth, G., Montanarella, L. \& Wesemael, B. van}. 2013. Prediction of soil organic carbon at the european scale by visible and near infrared reflectance spectroscopy. \emph{PloS one}, 8(6): e66409.
...
\bibitem[\citeproctext]{ref-takoutsing2022accounting}
\textbf{Takoutsing, B., Heuvelink, G.B., Stoorvogel, J.J., Shepherd, K.D. \& Aynekulu, E.} 2022. Accounting for analytical and proximal soil sensing errors in digital soil mapping. \emph{European Journal of Soil Science}, 73(2): e13226.

\bibitem[\citeproctext]{ref-viscarra2022diffuse}
\textbf{Viscarra Rossel, R.A., Behrens, T., Ben-Dor, E., Chabrillat, S., Demattê, J.A.M., Ge, Y., Gomez, C., Guerrero, C., Peng, Y., Ramirez-Lopez, L. \& others}. 2022. Diffuse reflectance spectroscopy for estimating soil properties: A technology for the 21st century. \emph{European Journal of Soil Science}, 73(4): e13271.

\bibitem[\citeproctext]{ref-viscarra2011soil}
\textbf{Viscarra Rossel, R., Chappell, A., De Caritat, P. \& McKenzie, N.} 2011. On the soil information content of visible--near infrared reflectance spectra. \emph{European Journal of Soil Science}, 62(3): 442--453.

\bibitem[\citeproctext]{ref-wadoux2025uncertainty}
\textbf{Wadoux, A.M.-C. \& Ramirez-Lopez, L.} 2025. Uncertainty of predictions in absorption spectroscopy: Modelling with quantile regression forest. \emph{Chemometrics and Intelligent Laboratory Systems}, 265: 105473.

\bibitem[\citeproctext]{ref-Wadoux2021integrated}
...
\bibitem[\citeproctext]{ref-wadoux2019multi}
\textbf{Wadoux, A.M.J.-C., Padarian, J. \& Minasny, B.} 2019. Multi-source data integration for soil mapping using deep learning. \emph{Soil}, 5(1): 107--119.

\bibitem[\citeproctext]{ref-Wadoux2025FAO}
\textbf{Wadoux, A.M.J.-C., Ramirez-Lopez, L., Ge, Y., Barra, I. \& Peng, Y.} 2025. \emph{A course on applied data analytics for soil analysis with infrared spectroscopy -- soil spectroscopy training manual 2}. Rome, Italy, Food; Agriculture Organization of the United Nations (FAO).
...
\bibitem[\citeproctext]{ref-van2022measurement}
\textbf{Westhuizen, S. van der, Heuvelink, G.B.M., Hofmeyr, D.P. \& Poggio, L.} 2022. Measurement error-filtered machine learning in digital soil mapping. \emph{Spatial Statistics}, 47: 100572.

\bibitem[\citeproctext]{ref-wright2017ranger}
\textbf{Wright, M.N. \& Ziegler, A.} 2017. Ranger: A fast implementation of random forests for high dimensional data in c++ and r. \emph{Journal of statistical software}, 77: 1--17.

\bibitem[\citeproctext]{ref-wu2025assessing}
\textbf{Wu, C.-Y., Wu, P.-H. \& Hseu, Z.-Y.} 2025. Assessing the robustness of VIS-NIR spectroscopy-based soil organic carbon prediction against four wet chemistry methods. \emph{Carbon Management}, 16(1): 2511337.
