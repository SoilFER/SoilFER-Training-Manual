...
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/new_frontcover.png}}
...
\section{Preparing Soil Observations}\label{preparing-soil-observations}

\subsection{Loading and exploring raw soil data}\label{loading-and-exploring-raw-soil-data}
...
\subsubsection{Basic data loading and structure}\label{basic-data-loading-and-structure}
...
\subsubsection{Data components and organization}\label{data-components-and-organization}
...
\subsection{Preparing site data for analysis}\label{preparing-site-data-for-analysis}
...
\subsubsection{Adding a unique row identifier}\label{adding-a-unique-row-identifier}
...
\subsubsection{Extracting and standardizing column names for sites}\label{extracting-and-standardizing-column-names-for-sites}
...
\subsubsection{Creating unique profile identifiers}\label{creating-unique-profile-identifiers}
...
\subsubsection{Remove duplicate site records}\label{remove-duplicate-site-records}
...
\subsection{Coordinate validation and correction}\label{coordinate-validation-and-correction}
...
\subsubsection{Check 1: Missing coordinates}\label{check-1-missing-coordinates}
...
\subsubsection{Check 2: Valid coordinate ranges}\label{check-2-valid-coordinate-ranges}
...
\subsection{Soil depth validation and correction}\label{soil-depth-validation-and-correction}
...
\subsubsection{Check 1: Missing depth boundaries}\label{check-1-missing-depth-boundaries}
...
\subsubsection{Check 2: Negative depth values}\label{check-2-negative-depth-values}
...
\subsubsection{Check 3: Zero-thickness intervals}\label{check-3-zero-thickness-intervals}
...
\subsubsection{Check 4: Invalid depth logic}\label{check-4-invalid-depth-logic}
...
\subsubsection{Check 5: Profiles without a surface horizon (top \textgreater{} 0)}\label{check-5-profiles-without-a-surface-horizon-top-0}
...
\subsection{Preparing lab data: harmonization and validation}\label{preparing-lab-data-harmonization-and-validation}
...
\subsubsection{Prepare and standardize laboratory columns}\label{prepare-and-standardize-laboratory-columns}
...
\subsubsection{Check 1: Check each property against feasible analytical thresholds}\label{check-1-check-each-property-against-feasible-analytical-thresholds}
...
\paragraph{Load property thresholds}\label{load-property-thresholds}
...
\paragraph{Identify out-of-bounds values}\label{identify-out-of-bounds-values}
...
\paragraph{Reporting out-of-bounds for properties and audit trail}\label{reporting-out-of-bounds-for-properties-and-audit-trail}
...
\subsubsection{Check 2: Texture validation}\label{check-2-texture-validation}
...
\subsubsection{Check 3: Correction of out-of-bounds laboratory values}\label{check-3-correction-of-out-of-bounds-laboratory-values}
...
\paragraph{Option 1: Targeted corrections (when error mechanism is known)}\label{option-1-targeted-corrections-when-error-mechanism-is-known}
...
\paragraph{\texorpdfstring{Option 2: Replace out-of-bounds values with \texttt{NA}}{Option 2: Replace out-of-bounds values with NA}}\label{option-2-replace-out-of-bounds-values-with-na}
...
\subsection{Resolving duplicated data in soil profiles}\label{resolving-duplicated-data-in-soil-profiles}
...
\subsubsection{Why duplicates occur}\label{why-duplicates-occur}
...
\subsubsection{Types of duplicate situations}\label{types-of-duplicate-situations}
...
\subsubsection{Check 1: Detect potential horizon duplicates within profiles}\label{check-1-detect-potential-horizon-duplicates-within-profiles}
...
\subsubsection{Resolving Profile Duplicates}\label{resolving-profile-duplicates}
...
\paragraph{Check 1: Average duplicated horizons (same depth intervals)}\label{check-1-average-duplicated-horizons-same-depth-intervals}
...
\paragraph{Check 2: Resolve ProfID for multiple depth sequences}\label{check-2-resolve-profid-for-multiple-depth-sequences}
...
\subsubsection{Remove profiles not starting at the surface}\label{remove-profiles-not-starting-at-the-surface}
...
\paragraph{Check 1: Remove profiles with incomplete surface coverage}\label{check-1-remove-profiles-with-incomplete-surface-coverage}
...
\subsection{Harmonizing data for DSM}\label{harmonizing-data-for-dsm}
...
\subsubsection{Depth standardization}\label{depth-standardization}
...
\subsubsection{Processing standardized data}\label{processing-standardized-data}
...
\subsubsection{Exporting standardized data for Digital Soil Mapping}\label{exporting-standardized-data-for-digital-soil-mapping}
...
\subsection{Preparing data for spectroscopy analyses}\label{preparing-data-for-spectroscopy-analyses}
...
\subsection{Summary of exported files}\label{summary-of-exported-files}
...
\section{Spatial Analysis in R}\label{spatial-analysis-in-r}
...
\subsection{Spatial Data Concepts}\label{spatial-data-concepts}
...
\subsection{Spatial Data Types: Vector vs Raster Data}\label{spatial-data-types-vector-vs-raster-data}
...
\subsection{\texorpdfstring{Spatial Packages in R: \texttt{\{sf\}} and \texttt{\{terra\}}}{Spatial Packages in R: \{sf\} and \{terra\}}}\label{spatial-packages-in-r-sf-and-terra}

\subsubsection{Installing and Loading Spatial Packages}\label{installing-and-loading-spatial-packages}
...
\subsubsection{\texorpdfstring{Working with Vector Data using \texttt{\{sf\}}}{Working with Vector Data using \{sf\}}}\label{working-with-vector-data-using-sf}
...
\paragraph{Importing and Exporting Vector Data}\label{importing-and-exporting-vector-data}
...
\paragraph{\texorpdfstring{Creating spatial vector objects from \texttt{CSV} files.}{Creating spatial vector objects from CSV files.}}\label{creating-spatial-vector-objects-from-csv-files.}
...
\paragraph{Exporting sf objects}\label{exporting-sf-objects}
...
\paragraph{Importing Shapefiles}\label{importing-shapefiles}
...
\subsubsection{Setting and Reprojecting CRS for vector data}\label{setting-and-reprojecting-crs-for-vector-data}
...
\paragraph{Buffering Plots (Influence Area)}\label{buffering-plots-influence-area}
...
\paragraph{Clip Soil Data to Defined Boundaries}\label{clip-soil-data-to-defined-boundaries}
...
\paragraph{Spatial Relationships and Joins}\label{spatial-relationships-and-joins}
...
##   LSAD CLASSFP MTFCC CSAFP CBSAFP METDIVFP FUNCSTAT AWATER    INTPTLAT     INTPTLON
## 1   06      H1 G4020  <NA>   <NA>     <NA>        A 140936 +38.9266259 -101.7711031
## 2   06      H1 G4020  <NA>   <NA>     <NA>        A 548248 +39.3513521 -101.7198590
## 3   06      H1 G4020  <NA>   <NA>     <NA>        A 140936 +38.9266259 -101.7711031
## 4   06      H1 G4020  <NA>   <NA>     <NA>        A  61987 +38.4819222 -101.3474341
## 5   06      H1 G4020  <NA>   <NA>     <NA>        A  61987 +38.4819222 -101.3474341
## 6   06      H1 G4020  <NA>   <NA>     <NA>        A  61987 +38.4819222 -101.3474341
##                               GlobalID                   geometry
## 1 5d2c40a3-7c2d-4f48-879b-11e53b1bc366 POINT (-101.9787 39.06988)
## 2 111d187f-9907-4a48-8678-a1bed6840eb9 POINT (-101.7259 39.27917)
## 3 5d2c40a3-7c2d-4f48-879b-11e53b1bc366  POINT (-101.667 38.83208)
## 4 c1ad0367-fe83-4ee2-91fb-256e33e36857  POINT (-101.536 38.50828)
## 5 c1ad0367-fe83-4ee2-91fb-256e33e36857  POINT (-101.534 38.50785)
## 6 c1ad0367-fe83-4ee2-91fb-256e33e36857 POINT (-101.5333 38.50486)
\end{verbatim}

\subsubsection{\texorpdfstring{Visualizing \texttt{\{sf\}} Objects}{Visualizing \{sf\} Objects}}\label{visualizing-sf-objects}
...
\section{Preparing Raster Covariates}\label{preparing-raster-covariates}
...
\subsubsection{Download Covariate Rasters}\label{download-covariate-rasters}
...
\subsubsection{Obtain Covariate Data from Google Earth Engine (GEE)}\label{obtain-covariate-data-from-google-earth-engine-gee}
...
\subsubsection{Running the GEE Script}\label{running-the-gee-script}
...
\subsection{\texorpdfstring{Working with Raster Data using \texttt{\{terra\}}}{Working with Raster Data using \{terra\}}}\label{working-with-raster-data-using-terra}
...
\subsubsection{Importing and Exporting Raster Data}\label{importing-and-exporting-raster-data}
...
\subsubsection{\texorpdfstring{Inspecting and Exploring \texttt{SpatRaster} Objects}{Inspecting and Exploring SpatRaster Objects}}\label{inspecting-and-exploring-spatraster-objects}
...
\subsubsection{Raster Visualization}\label{raster-visualization}
...
\subsubsection{Setting and Reprojecting CRS for Raster Data}\label{setting-and-reprojecting-crs-for-raster-data}
...
\subsubsection{Spatial Operations on Rasters}\label{spatial-operations-on-rasters}
...
\paragraph{Merge rasters (mosaic)}\label{merge-rasters-mosaic}
...
\paragraph{\texorpdfstring{Crop and mask to study area (\texttt{crop()}, \texttt{mask()})}{Crop and mask to study area (crop(), mask())}}\label{crop-and-mask-to-study-area-crop-mask}
...
\paragraph{\texorpdfstring{Resampling and alignment (\texttt{resample()})}{Resampling and alignment (resample())}}\label{resampling-and-alignment-resample}
...
\paragraph{Combine and remove rasters in a raster stack}\label{combine-and-remove-rasters-in-a-raster-stack}
...
\paragraph{Raster Algebra}\label{raster-algebra}
...
\paragraph{Combining environmental (Raster) and soil (Vector) information}\label{combining-environmental-raster-and-soil-vector-information}
...
\subsection{Calculation of Soil--Climate Rasters (Newhall Simulation Model)}\label{calculation-of-soilclimate-rasters-newhall-simulation-model}
...
\subsubsection{Prepare monthly average rasters of climatic data}\label{prepare-monthly-average-rasters-of-climatic-data}
...
\subsubsection{Prepare monthly average stack of climatic data}\label{prepare-monthly-average-stack-of-climatic-data}
...
\subsubsection{Prepare site descriptors that influence soil water balance and seasonality}\label{prepare-site-descriptors-that-influence-soil-water-balance-and-seasonality}
...
\subsubsection{Calculate NSM outputs}\label{calculate-nsm-outputs}
...
\subsubsection{Inspect and export results (compact outputs)}\label{inspect-and-export-results-compact-outputs}
...
\section{The GloSIS ISO 28258 database}\label{the-glosis-iso-28258-database}
...
\section{Standardization with the XLSX Template}\label{standardization-with-the-xlsx-template}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_plot} \caption{Plot Data Sheet.}\label{fig:plot-tab}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/element} \caption{Horizon vs Layer en Element Features.}\label{fig:element-fig}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_element} \caption{Element Data Sheet.}\label{fig:element-tab}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_specimen} \caption{Specimen Data Sheet.}\label{fig:specimen-tab}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_procedures} \caption{Procedures Sheet.}\label{fig:procedures-tab}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_metadata} \caption{Metadata Sheet.}\label{fig:metadata-tab}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/template_original} \caption{Original Data Sheet.}\label{fig:original-tab}
\end{figure}
...
\caption{\label{tab:table-soil-properties}Summary of soil properties and units of measurement in the GloSIS Onthology.}
\centering
\begin{tabular}[t]{l|l|l}
\hline
Category & Property & Unit\\
\hline
Soil organic components & Organic carbon & g/kg\\
\hline
Soil organic components & Total carbon & g/kg\\
\hline
Soil organic components & Inorganic carbon & g/kg\\
\hline
Soil organic components & Organic matter & g/kg\\
\hline
Soil organic components & Total carbonate equivalent & g/kg\\
\hline
Nitrogen and major nutrients & Total nitrogen & g/kg\\
\hline
Nitrogen and major nutrients & Phosphorus & \%\\
\hline
Nitrogen and major nutrients & Potassium & cmol/kg\\
\hline
Nitrogen and major nutrients & Phosphorus retention & g/hg\\
\hline
Exchangeable cations and acidity & Calcium & cmol/kg\\
\hline
Exchangeable cations and acidity & Magnesium & cmol/kg\\
\hline
Exchangeable cations and acidity & Sodium & cmol/kg\\
\hline
Exchangeable cations and acidity & Potassium & cmol/kg\\
\hline
Exchangeable cations and acidity & Aluminium & cmol/kg\\
\hline
Exchangeable cations and acidity & Hydrogen & cmol/kg\\
\hline
Exchangeable cations and acidity & Exchangeable acidity & cmol/kg\\
\hline
Exchangeable cations and acidity & Base saturation & \%\\
\hline
Exchangeable cations and acidity & Aluminium saturation & \%\\
\hline
Soil texture and coarse fraction & Sand & \%\\
\hline
Soil texture and coarse fraction & Silt & \%\\
\hline
Soil texture and coarse fraction & Clay & \%\\
\hline
Soil texture and coarse fraction & Coarse fragments & \%\\
\hline
Trace elements & Iron & \%\\
\hline
Trace elements & Manganese & cmol/kg\\
\hline
...
\hline
Soil physical properties & Bulk density (fine earth) & kg/dm³\\
\hline
Soil physical properties & Bulk density (whole soil) & kg/dm³\\
\hline
Soil physical properties & Porosity & m³/100 m³\\
\hline
Soil physical properties & Volumetric water content & m³/100 m³\\
\hline
Soil physical properties & Hydraulic conductivity & cm/h\\
\hline
Soil chemical properties & pH & pH\\
\hline
Soil chemical properties & Electrical conductivity & dS/m\\
\hline
Soil chemical properties & Soluble salts & cmol/L\\
\hline
Soil chemical properties & Cation exchange capacity & cmol/kg\\
\hline
Soil chemical properties & Effective cation exchange capacity & cmol/kg\\
\hline
Soil chemical properties & Ca/N ratio & ratio\\
\hline
Other soil constituents & Gypsum & \%\\
\hline
\end{tabular}
\end{table}

\section{The GloSIS ETL Platform}\label{the-glosis-etl-platform}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_mainscreen} \caption{Modules in the glosis-etl-js.}\label{fig:glosis-etl}
\end{figure}

\subsection{Installation and Setup of the glosis-etl-js}\label{installation-and-setup-of-the-glosis-etl-js}
...
\end{verbatim}
  ...
  \begin{figure}
  \includegraphics[width=1\linewidth]{images/module4/etl_clone} \caption{Cloning the repository.}\label{fig:clone}
  \end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/docker_containers} \caption{Docker Desktop showing running containers.}\label{fig:docker-desktop}
\end{figure}
...
\section{Running the GloSIS ETL}\label{running-the-glosis-etl}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_mainscreen} \caption{glosis-etl-js Harmonization module in first place.}\label{fig:etl-harmonization}
\end{figure}
...
\subsection{Data Harmonization}\label{data-harmonization}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization_steps} \caption{glosis-etl-js Harmonization module.}\label{fig:etl-harmonization-steps}
\end{figure}
...
\subsubsection{Data Upload Interface: Upload}\label{data-upload-interface-upload}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization1} \caption{Step 1: Data upload section.}\label{fig:etl-harmonization-step1}
\end{figure}
...
\subsubsection{Variable Mapping: Columns}\label{variable-mapping-columns}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization2} \caption{Step 2a: Parameter Mapping.}\label{fig:etl-harmonization-step2a}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization3} \caption{Step 2b: - Plot, Element and Analytical Mapping.}\label{fig:etl-harmonization-step2b}
\end{figure}

\subsubsection{Variable Mapping: Properties}\label{variable-mapping-properties}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization_all} \caption{Step 2c: Mapping analytical procedures and units of measurement.}\label{fig:etl-harmonization-step2c}
\end{figure}
...
\paragraph{Geographic Data Validation}\label{geographic-data-validation}
...
\paragraph{Soil Depth Validation}\label{soil-depth-validation}
...
\paragraph{Soil Properties Validation}\label{soil-properties-validation}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization_QC1} \caption{Step 3a: Reporting potential errors in the dataset.}\label{fig:etl-harmonization-QC1}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization_QC2} \caption{Step 3a: No errors identified in the dataset.}\label{fig:etl-harmonization-QC2}
\end{figure}

\paragraph{Data Correction and Resubmission}\label{data-correction-and-resubmission}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization9} \caption{Metadata Tab.}\label{fig:etl-harmonization-metadata}
\end{figure}

\subsubsection{Generate and Download the Harmonized Data}\label{generate-and-download-the-harmonized-data}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_harmonization8} \caption{Metadata Tab.}\label{fig:etl-harmonization-download}
\end{figure}
...
\subsubsection{Create a database to store your results}\label{create-a-database-to-store-your-results}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_injection1} \caption{Metadata Tab.}\label{fig:etl-standardization}
\end{figure}
...
\subsubsection{Inject the data into the created PostgreSQL database}\label{inject-the-data-into-the-created-postgresql-database}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_injection3} \caption{Standardization Upload and Injection tab.}\label{fig:etl-injection}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_viewer1} \caption{Visualization of the KSSL data in this Manual}\label{fig:etl-viewer1}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/etl_viewer2} \caption{Visualization of a subset of the KSSL data in this Manual}\label{fig:etl-viewer2}
\end{figure}
...
\section{Connecting PostgreSQL to QGIS}\label{connecting-postgresql-to-qgis}
...
{\centering \includegraphics[width=0.5\linewidth]{images/module4/qgis_connection} 

}

\caption{Connection information in the New PostGIS Connection}\label{fig:qgis-connection}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/qgis_dbmanager} \caption{Open Connection in DB Manager}\label{fig:qgis-dbmanager}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/qgis_dbquery} \caption{Open Connection in DB Manager}\label{fig:qgis-dbquery}
\end{figure}

A layer named \texttt{QueryLayer} will appear in the Layers Panel in QGIS (Figure \ref{fig:qgis-querylayer}).

\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/qgis_out} \caption{Sampling Points in QueryLayer}\label{fig:qgis-querylayer}
\end{figure}
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/module4/qgis_long} \caption{QueryLayer Table in long format}\label{fig:qgis-long}
\end{figure}

\section{Connecting PostgreSQL to R}\label{connecting-postgresql-to-r}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/example_covariate.png}}
\caption{\label{fig:example-covariate}Example environmental covariate layer from the 250m resolution stack.}
\end{figure}
...
## [28] "dtm_tpi_250m"          "dtm_twi_500m"
\end{verbatim}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/boruta_pH.png}}
\caption{\label{fig:boruta-plot}Boruta feature selection results. Green = confirmed important, red = rejected, yellow = tentative.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/varimp_pH.png}}
\caption{\label{fig:varimp-plot}Variable importance from the trained Random Forest model.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/scatterplot_pH.png}}
\caption{\label{fig:scatterplot}Observed vs Predicted pH from cross-validation. The red line shows perfect 1:1 prediction.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_mean_pH.png}}
\caption{\label{fig:map-mean}Predicted mean pH across the study area.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_sd_pH.png}}
\caption{\label{fig:map-sd}Prediction uncertainty (SD) - higher values indicate more uncertain predictions.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_cv_pH.png}}
\caption{\label{fig:map-cv}Coefficient of Variation (\%) - relative uncertainty.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/confusion_matrix_Clay_pH_Class.png}}
\caption{\label{fig:confusion-matrix}Confusion matrix showing predicted vs observed classes. Diagonal cells are correct predictions.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_class_max_prob.png}}
\caption{\label{fig:map-class-max}Predicted class based on maximum probability.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_max_prob_value.png}}
\caption{\label{fig:map-prob-max}Maximum class probability - higher values indicate more confident predictions.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_uncertainty_nominal.png}}
\caption{\label{fig:map-uncertainty-nom}Classification uncertainty (1 - max probability).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_class_confident.png}}
\caption{\label{fig:map-confident}Only predictions where max probability \textgreater= 0.4 are shown.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/ordinal_confusion_pH_Class.png}}
\caption{\label{fig:ordinal-confusion}Ordinal confusion matrix. Errors near the diagonal are minor; errors far from diagonal are severe.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_latent_pH_Class.png}}
\caption{\label{fig:map-latent}Continuous `latent' predictions before reclassification to ordinal classes.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_class_pH_Class.png}}
\caption{\label{fig:map-class-ord}Ordinal classes derived from latent predictions using midpoint thresholds.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_sd_pH_Class.png}}
\caption{\label{fig:map-sd-ord}Prediction uncertainty (SD) from the ordinal regression model.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_instability_pH_Class.png}}
\caption{\label{fig:map-instability}Class instability - high values where predictions are near class boundaries.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_DI_pH.png}}
\caption{\label{fig:map-di}Dissimilarity Index - how different each location is from training data in covariate space.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_AOA_pH.png}}
\caption{\label{fig:map-aoa-mask}Area of Applicability mask. Green = within AOA (trustworthy), Grey = outside AOA (unreliable).}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_LPD_pH.png}}
\caption{\label{fig:map-lpd}Local Point Density - number of training points supporting predictions at each location.}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{SoilFER-Integrated-soil-information-for-decision-making_files/images/module5/map_mean_pH_masked_AOA.png}}
\caption{\label{fig:map-masked-aoa}pH predictions only shown within the Area of Applicability.}
\end{figure}
