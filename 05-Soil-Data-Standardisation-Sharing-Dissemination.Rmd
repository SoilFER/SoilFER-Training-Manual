...
- publish the **raster maps** you produce from soil observations and environmental covariates (the production of these maps is covered in the module 3 - **Digital Soil Mapping**);
...
```{r sis-profile-upload1, echo=FALSE, out.width="100%", fig.cap="Step 1: Selecting and uploading the CSV file."}
...
In this example, we will generate a new raster (KSSL threats) for the KANSAS project, that combines the pH, organic carbon, bulk density, and sand content layers. Each input raster is first reclassified into a binary (0-1) map based on the thresholds defined in the table. Finally, these binary layers are summed to produce the overall threat map.
